...
TEST_F(EnhancedSystemTest, SequencerExecutionStrategies) {
    ...
    sequencer_->setExecutionStrategy(
        ExposureSequence::ExecutionStrategy::Sequential);
    ASSERT_EQ(sequencer_->getExecutionStrategy(),
              ExposureSequence::ExecutionStrategy::Sequential);

    // Test parallel execution
    sequencer_->setExecutionStrategy(
        ExposureSequence::ExecutionStrategy::Parallel);
    ASSERT_EQ(sequencer_->getExecutionStrategy(),
              ExposureSequence::ExecutionStrategy::Parallel);

    // Test adaptive execution
    sequencer_->setExecutionStrategy(
        ExposureSequence::ExecutionStrategy::Adaptive);
    ASSERT_EQ(sequencer_->getExecutionStrategy(),
              ExposureSequence::ExecutionStrategy::Adaptive);

    // Test priority execution
    sequencer_->setExecutionStrategy(
        ExposureSequence::ExecutionStrategy::Priority);
    ASSERT_EQ(sequencer_->getExecutionStrategy(),
              ExposureSequence::ExecutionStrategy::Priority);
}
...
TEST_F(EnhancedSystemTest, SchedulingStrategies) {
    ...
    sequencer_->setSchedulingStrategy(
        ExposureSequence::SchedulingStrategy::FIFO);

    // Test Priority scheduling
    sequencer_->setSchedulingStrategy(
        ExposureSequence::SchedulingStrategy::Priority);

    // Test Dependencies scheduling
    sequencer_->setSchedulingStrategy(
        ExposureSequence::SchedulingStrategy::Dependencies);
...
TEST_F(EnhancedSystemTest, TaskParameterValidation) {
    ...
    json validParams = {{"exposure", 30.0},
                        {"type", "light"},
                        {"binning", 1},
                        {"gain", 100},
                        {"offset", 10}};
...
TEST_F(EnhancedSystemTest, ScriptTaskCreation) {
    ...
    auto scriptTask = factory.createTask("RunScript", "test_script",
                                         json{{"script_path", "/tmp/test.py"},
                                              {"script_type", "python"},
                                              {"timeout", 5000}});
...
TEST_F(EnhancedSystemTest, ErrorHandlingAndRecovery) {
    ...
    sequencer_->setRecoveryStrategy(
        ExposureSequence::RecoveryStrategy::Alternative);
...
TEST_F(EnhancedSystemTest, ProgressCallback) {
    ...
    sequencer_->setProgressCallback(
        [&callbackCalled, &lastProgress](const json& progress) {
            callbackCalled = true;
