...
protected:
    void SetUp() override {
        ...
        task_ = std::make_unique<ScriptTask>("test_script_task",
...
TEST_F(ScriptTaskTest, ConstructionWithConfigs) {
    EXPECT_NO_THROW(ScriptTask("config_task", scriptConfigPath_.string(),
                               analyzerConfigPath_.string()));
}
...
TEST_F(ScriptTaskTest, AddPreExecutionHook) {
    ...
    task_->addPreExecutionHook("hook_test",
                               [&](const std::string&) { hookCalled = true; });
...
TEST_F(ScriptTaskTest, SetRetryStrategy) {
    ...
    EXPECT_NO_THROW(
        task_->setRetryStrategy("strategy_test", RetryStrategy::Exponential));
}
...
TEST_F(ScriptTaskTest, AddEventListener) {
    ...
    task_->addEventListener("script_complete",
                            [&](const json&) { eventFired = true; });
