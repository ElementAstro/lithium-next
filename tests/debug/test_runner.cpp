...
// Integration tests
TEST_F(IntegrationTestSuite, FullWorkflow) { TestFullWorkflow(); }

TEST_F(IntegrationTestSuite, ComponentInteraction) {
    TestComponentInteraction();
}

TEST_F(IntegrationTestSuite, ErrorPropagation) { TestErrorPropagation(); }
...
TEST_F(PerformanceBenchmarks, DISABLED_MemoryUsage) { BenchmarkMemoryUsage(); }
...
public:
    void TestMassiveComponentRegistration() {
        ...
        for (int i = 0; i < numComponents; ++i) {
            ...
            EXPECT_CALL(*component, getName())
                .WillRepeatedly(
                    ::testing::Return(std::format("StressComponent{}", i)));
            ...
            ASSERT_TRUE(result.has_value())
                << std::format("Failed to register component {}", i);
        }

        // Verify all components are registered
        auto allComponents = manager_->getAllComponents();
        EXPECT_EQ(allComponents.size(), numComponents)
    ...
    void TestHighConcurrency() {
                ...
                for (int j = 0; j < operationsPerThread; ++j) {
                    ...
                    EXPECT_CALL(*component, getName())
                        .WillRepeatedly(::testing::Return(
                            std::format("ConcurrentComponent{}_{}", i, j)));
                    EXPECT_CALL(*component, initialize())
                        .WillOnce(::testing::Return(Result<void>{}));
                    EXPECT_CALL(*component, shutdown())
                        .WillOnce(::testing::Return(Result<void>{}));
                    ...
                    if (regResult.has_value()) {
                        successfulOperations.fetch_add(
                        ...
                        [[maybe_unused]] auto unregResult =
                            manager_->unregisterComponent(component);
                    }
                }
            });
        }
        ...
        std::cout << std::format(
            "Concurrent operations: {}/{} successful ({:.2f}%)\n", successful,
            total, successRate);
...
TEST_F(StressTestSuite, DISABLED_HighConcurrency) { TestHighConcurrency(); }
...
TEST_F(AsyncOperationTest, AsyncCommandChecking) {
    ...
    struct AsyncTestRule {
        ...
        DebugTask<result_type> checkAsync(std::string_view command, size_t line,
                                          size_t column) const {
    ...
    auto regResult =
        checker_->registerAsyncRule("async_test_rule", AsyncTestRule{});
    ASSERT_TRUE(regResult.has_value());
    ...
    EXPECT_GE(duration.count(), 50)
        << "Should take at least 50ms due to async rule delay";
}
