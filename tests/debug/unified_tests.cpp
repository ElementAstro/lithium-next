...
void UnifiedDebugManagerTestSuite::TestInitialization() {
    ...
    EXPECT_FALSE(doubleInitResult.has_value())
...
void UnifiedDebugManagerTestSuite::TestShutdown() {
    ...
    EXPECT_FALSE(manager_->isActive())
        << "Manager should not be active after shutdown";

    // Test double shutdown
    auto doubleShutdownResult = manager_->shutdown();
    EXPECT_FALSE(doubleShutdownResult.has_value())
...
void UnifiedDebugManagerTestSuite::TestReset() {
    ...
    EXPECT_TRUE(components.empty())
        << "All components should be cleared after reset";
}
...
void UnifiedDebugManagerTestSuite::TestRegisterComponent() {
    ...
    EXPECT_TRUE(component.has_value())
        << "Registered component should be retrievable";
...
void UnifiedDebugManagerTestSuite::TestUnregisterComponent() {
    ...
    EXPECT_TRUE(result.has_value())
        << "Component unregistration should succeed";

    // Verify component is removed
    auto component = manager_->getComponent("TestComponent");
    EXPECT_FALSE(component.has_value())
        << "Unregistered component should not be retrievable";
    ...
    EXPECT_FALSE(nonExistentResult.has_value())
        << "Unregistering non-existent component should fail";
}
...
void UnifiedDebugManagerTestSuite::TestErrorReporting() {
    ...
    DebugError testError{ErrorCode::RUNTIME_ERROR, "Test error message",
                         ErrorCategory::GENERAL, ErrorSeverity::ERROR};

    manager_->setErrorReporter(mockReporter_);

    EXPECT_CALL(*mockReporter_, reportError(::testing::_)).Times(1);
    ...
    EXPECT_CALL(*mockReporter_, reportException(::testing::_)).Times(1);
...
void UnifiedDebugManagerTestSuite::TestConcurrentAccess() {
    ...
    EXPECT_EQ(successCount.load(), numThreads)
        << "All unique components should register successfully";
}
...
void OptimizedTerminalTestSuite::TestInitialization() {
    ...
    EXPECT_TRUE(terminal_->isActive())
        << "Terminal should be active after initialization";
...
void OptimizedTerminalTestSuite::TestCommandRegistration() {
    ...
    auto result = terminal_->registerCommand(
        "test", [](std::span<const std::any> args) -> Result<std::string> {
    ...
    EXPECT_NE(testIt, commands.end())
        << "Test command should be in registered commands";

    // Test duplicate registration
    auto duplicateResult = terminal_->registerCommand(
        "test", [](std::span<const std::any> args) -> Result<std::string> {
...
void OptimizedTerminalTestSuite::TestCommandExecution() {
    ...
    auto regResult = terminal_->registerCommand(
        "hello",
        [expectedOutput](std::span<const std::any> args)
            -> Result<std::string> { return expectedOutput; });
    ASSERT_TRUE(regResult.has_value());

    // Execute the command
    auto result = terminal_->executeCommand("hello");
    EXPECT_TRUE(result.has_value()) << "Command execution should succeed";
    EXPECT_EQ(result.value(), expectedOutput)
...
void OptimizedTerminalTestSuite::TestAsyncCommandExecution() {
    ...
    auto regResult = terminal_->registerAsyncCommand(
        "async_hello",
        [expectedOutput](
    ...
    EXPECT_EQ(result.value(), expectedOutput)
        << "Async command should return expected output";
}
...
void OptimizedTerminalTestSuite::TestCommandHistory() {
    ...
    EXPECT_TRUE(clearedHistory.empty())
...
void OptimizedCheckerTestSuite::TestInitialization() {
    ...
    EXPECT_TRUE(checker_->isActive())
        << "Checker should be active after initialization";
...
void OptimizedCheckerTestSuite::TestRuleRegistration() {
    ...
    struct TestRule {
        ...
        result_type check(std::string_view command, size_t line,
                          size_t column) const {
            if (command.find("test") != std::string_view::npos) {
                return CheckError{.message = "Test rule triggered",
...
void OptimizedCheckerTestSuite::TestCommandChecking() {
    ...
    EXPECT_FALSE(safeResult->hasCriticalErrors())
        << "Safe command should not have critical errors";

    // Test checking a dangerous command
    auto dangerousResult = checker_->checkCommand("rm -rf /");
    EXPECT_TRUE(dangerousResult.has_value())
        << "Dangerous command check should succeed";
    EXPECT_TRUE(dangerousResult->hasErrors())
        << "Dangerous command should have errors";
}
...
void OptimizedCheckerTestSuite::TestSecurityAnalysis() {
    ...
    EXPECT_GT(riskResult.value(), 0.0)
        << "Dangerous command should have positive risk";
...
void OptimizedCheckerTestSuite::TestBatchChecking() {
    ...
    std::vector<std::string> commands = {"echo hello", "ls -la", "rm -rf /",
                                         "cat /etc/passwd"};
    ...
    EXPECT_EQ(results->size(), commands.size())
        << "Should get result for each command";
...
void ErrorHandlingTestSuite::TestErrorCreation() {
    DebugError error{ErrorCode::RUNTIME_ERROR, "Test error message",
                     ErrorCategory::GENERAL, ErrorSeverity::ERROR};
...
void ErrorHandlingTestSuite::TestDebugException() {
    DebugError error{ErrorCode::RUNTIME_ERROR, "Test exception",
                     ErrorCategory::GENERAL, ErrorSeverity::ERROR};
...
void ErrorHandlingTestSuite::TestRecoveryStrategies() {
    ...
    auto strategy =
        [&strategyCalled](const DebugError& error) -> RecoveryAction {
        strategyCalled = true;
    ...
    DebugError error{ErrorCode::RUNTIME_ERROR, "Test", ErrorCategory::GENERAL,
                     ErrorSeverity::ERROR};
    auto action = recoveryManager->attemptRecovery(error);

    EXPECT_TRUE(strategyCalled) << "Recovery strategy should be called";
    EXPECT_EQ(action, RecoveryAction::RETRY)
        << "Should return expected recovery action";
}
...
void ErrorHandlingTestSuite::TestErrorAggregation() {
    ...
    for (int i = 0; i < 10; ++i) {
        DebugError error{ErrorCode::RUNTIME_ERROR, std::format("Error {}", i),
                         ErrorCategory::GENERAL, ErrorSeverity::ERROR};
    ...
    EXPECT_GT(stats.errorsByCode[ErrorCode::RUNTIME_ERROR], 0)
...
void ErrorHandlingTestSuite::TestResultType() {
    ...
    EXPECT_TRUE(successResult.has_value())
        << "Success result should have value";
    ...
    EXPECT_FALSE(errorResult.has_value())
        << "Error result should not have value";
    EXPECT_EQ(errorResult.error().code, ErrorCode::RUNTIME_ERROR)
...
void IntegrationTestSuite::TestFullWorkflow() {
    ...
    auto regResult = terminal_->registerCommand(
        "check", [this](std::span<const std::any> args) -> Result<std::string> {
            if (args.empty()) {
                return unexpected(DebugError{
                    ErrorCode::INVALID_ARGUMENT, "Command to check required",
                    ErrorCategory::TERMINAL, ErrorSeverity::ERROR});
            }
            ...
            try {
                ...
                return unexpected(
                    DebugError{ErrorCode::TYPE_ERROR, "Invalid argument type",
                               ErrorCategory::TERMINAL, ErrorSeverity::ERROR});
            }
        });

    ASSERT_TRUE(regResult.has_value())
        << "Check command registration should succeed";
    ...
    EXPECT_TRUE(result.has_value())
        << "Integrated command should execute successfully";
...
void IntegrationTestSuite::TestComponentInteraction() {
    ...
    EXPECT_GE(components.size(), 2)
        << "Manager should have registered components";
    ...
    EXPECT_TRUE(terminalComponent.has_value())
        << "Terminal should be retrievable from manager";
    EXPECT_TRUE(checkerComponent.has_value())
        << "Checker should be retrievable from manager";
}
...
void IntegrationTestSuite::TestErrorPropagation() {
    ...
    auto regResult = terminal_->registerCommand(
        "throw", [](std::span<const std::any> args) -> Result<std::string> {
    ...
    EXPECT_EQ(result.error().code, ErrorCode::RUNTIME_ERROR)
...
void PerformanceBenchmarks::BenchmarkComponentRegistration() {
    ...
    for (int i = 0; i < numComponents; ++i) {
        ...
        EXPECT_CALL(*component, getName())
            .WillRepeatedly(
                ::testing::Return(std::format("BenchComponent{}", i)));
        EXPECT_CALL(*component, initialize())
            .WillOnce(::testing::Return(Result<void>{}));
        ...
        ASSERT_TRUE(result.has_value())
            << std::format("Component {} registration failed", i);
    }
    ...
    std::cout << std::format(
        "Registered {} components in {}μs (avg: {:.2f}μs per component)\n",
...
void PerformanceBenchmarks::BenchmarkCommandExecution() {
    ...
    auto regResult = terminal_->registerCommand(
        "perf_test", [](std::span<const std::any> args) -> Result<std::string> {
    ...
    std::cout << std::format(
        "Executed {} commands in {}μs (avg: {:.2f}μs per command)\n",
...
void PerformanceBenchmarks::BenchmarkCommandChecking() {
    ...
    const std::vector<std::string> testCommands = {
        "echo hello world", "ls -la /home", "grep pattern file.txt",
        "find /usr -name '*.so'", "ps aux | grep process"};
    ...
    for (int i = 0; i < numIterations; ++i) {
        for (const auto& command : testCommands) {
            ...
            ASSERT_TRUE(result.has_value())
                << std::format("Check failed for: {}", command);
        }
    }
    ...
    std::cout << std::format(
        "Performed {} checks in {}μs (avg: {:.2f}μs per check)\n", totalChecks,
        duration.count(), static_cast<double>(duration.count()) / totalChecks);
...
void PerformanceBenchmarks::BenchmarkMemoryUsage() {
    ...
    // In a real implementation, you would use tools like valgrind or custom
    // memory tracking
