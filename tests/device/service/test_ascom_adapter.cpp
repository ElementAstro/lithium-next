...
TEST_F(ASCOMAdapterTest, RegisterDevice) {
    auto defaultAdapter = std::dynamic_pointer_cast<DefaultASCOMAdapter>(adapter_);
...
TEST_F(ASCOMAdapterTest, EventCallback) {
    ...
    // Callback is registered but events would be emitted during actual operations
...
TEST_F(ASCOMAdapterTest, FactoryCreateDefaultAdapter) {
    ...
    auto defaultAdapter = std::dynamic_pointer_cast<DefaultASCOMAdapter>(adapter);
