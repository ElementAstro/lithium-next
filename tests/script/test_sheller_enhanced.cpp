...
TEST_F(EnhancedScriptManagerTest, DetectAutoLanguage) {
    ...
    EXPECT_TRUE(language == ScriptLanguage::Shell || language == ScriptLanguage::Auto);
}
...
TEST_F(EnhancedScriptManagerTest, DiscoverRecursive) {
    ...
    size_t countNonRecursive = manager_->discoverScripts(testDir_, {".py"}, false);
...
TEST_F(EnhancedScriptManagerTest, DiscoverFromNonexistentDirectory) {
    size_t count = manager_->discoverScripts("/nonexistent/path", {".py"}, false);
    EXPECT_EQ(count, 0);
}
...
TEST_F(EnhancedScriptManagerTest, GetResourceUsage) {
    ...
    EXPECT_TRUE(usage.contains("running_scripts") || usage.contains("total_scripts"));
}
...
TEST_F(EnhancedScriptManagerTest, ExecutePipelineWithContext) {
    ...
    auto results = manager_->executePipeline({"context_pipe1", "context_pipe2"}, context, true);
...
TEST_F(EnhancedScriptManagerTest, ScriptStatisticsAfterExecution) {
    ...
    EXPECT_TRUE(stats.contains("execution_count") || stats.contains("executionCount"));
...
TEST_F(EnhancedScriptManagerTest, RegisterPythonScriptWithConfig) {
    ...
    EXPECT_NO_THROW(manager_->registerPythonScriptWithConfig("py_test", config));
...
TEST_F(EnhancedScriptManagerTest, PostExecutionHookCalled) {
    ...
    manager_->addPostExecutionHook("post_hook_test", 
...
TEST_F(EnhancedScriptManagerTest, MultipleHooks) {
    ...
    manager_->addPreExecutionHook("multi_hook", [&](const std::string&) {
        hookCount++;
    });
