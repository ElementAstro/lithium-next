...
protected:
    void SetUp() override { detector = std::make_unique<PlatformDetector>(); }

    void TearDown() override { detector.reset(); }
...
TEST_F(PlatformDetectorTest, GetCurrentPlatform) {
    ...
    bool isKnownPlatform = platform.find("Windows") != std::string::npos ||
...
TEST_F(PlatformDetectorTest, GetDefaultPackageManager) {
    ...
    bool isKnown = std::find(knownPkgMgrs.begin(), knownPkgMgrs.end(),
                             pkgMgr) != knownPkgMgrs.end();
    EXPECT_TRUE(isKnown);
}
