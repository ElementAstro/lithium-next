...
class DotNetManager:
            ...
            logger.info(
                ...
                extra={
                    ...
                    "latest_version": (
                        ...
                        if system_info.latest_installed_version
                        else None
                    ),
                },
            )
    ...
    def check_installed(self, version_key: str) -> bool:
        ...
        try:
            ...
            is_installed = (
                isinstance(installed_release, int)
                and installed_release >= version_info.release
            )
            ...
            with self._registry_key(self.NET_FRAMEWORK_REGISTRY_PATH) as ndp_key:
                ...
                for i in range(key_count):
                    try:
                        ...
                        version_path = (
                            f"{self.NET_FRAMEWORK_REGISTRY_PATH}\\{version_key_name}"
                        )
                        ...
                        for subkey in subkeys_to_check:
                            ...
                            try:
                                release = self._query_registry_value(
                                    full_path, "Release"
                                )
                                version_str = self._query_registry_value(
                                    full_path, "Version"
                                )
    ...
    async def download_file_async(
        ...
        if (
            ...
            and await self.verify_checksum_async(output_path, expected_checksum)
        ):
        ...
        try:
            ...
            speed_mbps = (
                (downloaded / (1024 * 1024)) / download_time if download_time > 0 else 0
            )
    ...
    def install_software(
        self, installer_path: Path, quiet: bool = False, timeout_seconds: int = 3600
    ) -> InstallationResult:
        """
        ...
        if not installer_path.exists():
            raise InstallationError(
                f"Installer not found: {installer_path}", installer_path=installer_path
            )
