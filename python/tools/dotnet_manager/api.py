...
async def download_file_async(
    ...
    if not url or not isinstance(url, str):
        raise DotNetManagerError(
            "URL must be a non-empty string", error_code="INVALID_URL", url=url
        )
    ...
    result = await manager.download_file_async(
        url, path, expected_checksum, show_progress
    )
    end_time = time.time()
...
async def verify_checksum_async(
    ...
    manager = DotNetManager()
    result = await manager.verify_checksum_async(
        Path(file_path), expected_checksum, algorithm
    )
...
def install_software(
    installer_path: str, quiet: bool = True, timeout_seconds: int = 3600
) -> InstallationResult:
    """
...
def get_version_info(version_key: str) -> Optional[DotNetVersion]:
    ...
    if version_info:
        logger.debug(
            ...
            extra={
                "version_key": version_key,
                "is_downloadable": version_info.is_downloadable,
            },
        )
    else:
...
def download_file(
    ...
    try:
        return asyncio.run(
            download_file_async(url, output_path, expected_checksum, show_progress)
        )
    except Exception as e:
...
def verify_checksum(
    ...
    try:
        return asyncio.run(
            verify_checksum_async(file_path, expected_checksum, algorithm)
        )
    except Exception as e:
