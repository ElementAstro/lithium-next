...
try:
    ...
    try:
        ...
        raise ImportError("Neither tomllib (Python 3.11+) nor tomli is installed. Please install tomli for TOML parsing.")
...
def _dict_to_toml(data: Dict[str, Any], indent: int = 0) -> str:
    ...
    for key, value in data.items():
        if isinstance(value, dict):
            ...
            lines.append(f"{indent_str}{key} = {str(value).lower() if isinstance(value, bool) else value}")
        elif value is None:
...
from .cert_types import (
    CertificateOptions, CertificateType, HashAlgorithm, KeySize,
    ...
    def validate_profiles(cls, v: Dict[str, Any]) -> Dict[str, ProfileConfig]:
        ...
        for name, profile_data in v.items():
            if isinstance(profile_data, dict):
                try:
                    validated_profiles[name] = ProfileConfig.model_validate(profile_data)
...
class EnhancedConfigManager:
    ...
    async def get_options_async(
        ...
        filtered_dict = {
            k: v for k, v in merged_dict.items() 
    ...
    async def add_profile_async(
        self,
        name: str,
        profile_config: Union[ProfileConfig, Dict[str, Any]]
    ) -> None:
        """
