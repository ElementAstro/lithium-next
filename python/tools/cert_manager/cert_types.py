...
class CertificateOptions(BaseModel):
    ...
    hostname: str = Field(
        description="Primary hostname for the certificate", min_length=1, max_length=253
    )
    cert_dir: Path = Field(description="Directory to store certificate files")
    key_size: KeySize = Field(
        default=KeySize.SIZE_2048, description="RSA key size in bits"
    )
    ...
    cert_type: CertificateType = Field(
        default=CertificateType.SERVER, description="Type of certificate to generate"
    )
    ...
    locality: Optional[str] = Field(
        default=None, min_length=1, max_length=128, description="Locality or city name"
    )
    organization: Optional[str] = Field(
        default=None, min_length=1, max_length=128, description="Organization name"
    )
    ...
    def validate_hostname(cls, v: str) -> str:
        ...
        if "." in v:
            ...
            if len(parts[-1]) < 2:
                logger.warning(
                    f"Hostname '{v}' has a short TLD, which may not be valid"
                )
    ...
    def validate_certificate_options(self) -> CertificateOptions:
        ...
        if self.cert_type.is_ca_type and self.path_length is None:
            self.path_length = (
                0 if self.cert_type == CertificateType.INTERMEDIATE else None
            )
...
class CertificateResult(BaseModel):
    ...
    fingerprint: Optional[str] = Field(
        default=None, description="Certificate fingerprint (SHA256)"
    )
    ...
    not_valid_after: Optional[datetime.datetime] = Field(
        default=None, description="Certificate validity end date"
    )
...
class CSRResult(BaseModel):
    ...
    subject: Optional[str] = Field(
        default=None, description="CSR subject distinguished name"
    )
    public_key_info: Optional[str] = Field(
        default=None, description="Public key information"
    )
...
class SignOptions(BaseModel):
    ...
    valid_days: int = Field(
        default=365, ge=1, le=7300, description="Certificate validity period in days"
    )
    hash_algorithm: HashAlgorithm = Field(
        default=HashAlgorithm.SHA256, description="Hash algorithm for signing"
    )
...
class RevokeOptions(BaseModel):
    ...
    reason: RevocationReason = Field(
        default=RevocationReason.UNSPECIFIED, description="Reason for revocation"
    )
    revocation_date: Optional[datetime.datetime] = Field(
        default=None, description="Revocation date (defaults to current time)"
    )
...
class RevokedCertInfo(BaseModel):
    ...
    revocation_date: datetime.datetime = Field(
        description="When certificate was revoked"
    )
    reason: Optional[x509.ReasonFlags] = Field(
        default=None, description="Revocation reason"
    )
    invalidity_date: Optional[datetime.datetime] = Field(
        default=None, description="Date when certificate became invalid"
    )
