...
class CertManagerPyBindAdapter:
    ...
    def create_cert(
        ...
        """
        logger.info(f"C++ binding: Creating certificate for {hostname} in {cert_dir}")
        try:
            ...
            return {"success": False, "cert_path": "", "key_path": "", "error": str(e)}
    ...
    def renew_cert(
        ...
        try:
            ...
            return {"success": True, "new_cert_path": str(new_path), "error": ""}
        except Exception as e:
            logger.exception(f"Error in renew_cert: {e}")
            return {"success": False, "new_cert_path": "", "error": str(e)}
    ...
    def export_cert(
        ...
        try:
            ...
            return {"success": False, "pfx_path": "", "error": str(e)}
    ...
    async def get_cert_info_async(cert_path: str) -> Dict:
        ...
        try:
            ...
            details = await loop.run_in_executor(
                None, lambda: get_cert_details(Path(cert_path))
            )
            return {
                "success": True,
                "details": {
    ...
    async def check_expiry_async(cert_path: str, warning_days: int = 30) -> Dict:
        ...
        try:
            ...
            is_expiring, days_remaining = await loop.run_in_executor(
                None, lambda: check_cert_expiry(Path(cert_path), warning_days)
            )
            return {
                "success": True,
                "is_expiring": is_expiring,
