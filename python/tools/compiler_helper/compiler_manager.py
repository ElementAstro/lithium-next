...
class CompilerManager:
    ...
    async def detect_compilers_async(self) -> Dict[str, Compiler]:
        ...
        for spec in self._compiler_specs:
            task = asyncio.create_task(
                self._detect_compiler_async(spec),
                name=f"detect_{spec.name}"
            )
    ...
    async def _detect_compiler_async(self, spec: CompilerSpec) -> Optional[Compiler]:
        ...
        try:
            ...
            version = await self._get_compiler_version_async(compiler_path, spec.compiler_type)
    ...
    def _create_compiler_features(
        ...
        if compiler_type in {CompilerType.GCC, CompilerType.CLANG}:
            ...
            if compiler_type == CompilerType.GCC and len(version_tuple) >= 2 and version_tuple >= (11, 0):
                features.supports_modules = True
                features.supports_concepts = True
                features.supported_cpp_versions.add(CppVersion.CPP23)
            
            elif compiler_type == CompilerType.CLANG and len(version_tuple) >= 2 and version_tuple >= (16, 0):
                features.supports_modules = True
                features.supports_concepts = True
                features.supported_cpp_versions.add(CppVersion.CPP23)
    ...
    def _find_msvc(self) -> Optional[str]:
        ...
        vswhere_path = Path(os.environ.get("ProgramFiles(x86)", "")) / \
                ...
                if tools_path.exists():
                    ...
                    if versions:
                        ...
                        for host_arch, target_arch in [("x64", "x64"), ("x86", "x86")]:
                            cl_path = tools_path / latest_version / "bin" / \
                                     f"Host{host_arch}" / target_arch / "cl.exe"
                            if cl_path.exists():
                                return str(cl_path)
    ...
    def list_compilers(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            name: {
                ...
                "cpp_versions": [v.value if hasattr(v, 'value') else str(v) for v in compiler.config.features.supported_cpp_versions],
                "features": {
