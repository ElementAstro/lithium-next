...
def mock_diagnostic_parser():
    ...
    parser.parse_diagnostics.return_value = (
        [],
        [],
        [],
    )  # Default: no errors, no warnings, no notes
    return parser
...
def mock_compiler_config_gcc(mock_compiler_config_base):
    ...
    config_data["features"].supported_cpp_versions = {
        CppVersion.CPP17,
        CppVersion.CPP20,
        CppVersion.CPP23,
    }
...
def compiler_instance(
    mock_compiler_config_gcc,
    mock_process_manager,
    mock_diagnostic_parser,
    ...
    with (
        patch(
            "tools.compiler_helper.compiler.ProcessManager",
            return_value=mock_process_manager,
        ),
        patch(
            "tools.compiler_helper.compiler.DiagnosticParser",
            return_value=mock_diagnostic_parser,
        ),
        patch(
            "tools.compiler_helper.compiler.CompilerMetrics",
            return_value=mock_compiler_metrics,
        ),
        patch("os.access", return_value=True),
        patch("pathlib.Path.exists", return_value=True),
    ):  # Simulate compiler executable exists and is executable
        compiler = Compiler(mock_compiler_config_gcc)
        yield compiler


@pytest.fixture
def compiler_instance_msvc(
...
@pytest.fixture
def compiler_instance_clang(
    mock_compiler_config_clang,
    mock_process_manager,
    mock_diagnostic_parser,
    mock_compiler_metrics,
    mocker,
...
@pytest.mark.asyncio
async def test_compile_async_success(
    compiler_instance,
    mock_process_manager,
    mock_diagnostic_parser,
    mock_compiler_metrics,
    ...
    mock_process_manager.run_command_async.assert_called_once_with(
    ...
    mock_compiler_metrics.record_compilation.assert_called_once_with(
        True, 0.1, is_link=False
    )  # Duration is in seconds for metrics


@pytest.mark.asyncio
async def test_compile_async_command_failure(
    compiler_instance,
    mock_process_manager,
    mock_diagnostic_parser,
    mock_compiler_metrics,
    ...
    # Mock Path.exists for the output file check after command runs
    mocker.patch.object(
        Path, "exists", side_effect=lambda: False
    ...
    mock_compiler_metrics.record_compilation.assert_called_once_with(
        False, 0.1, is_link=False
    )


@pytest.mark.asyncio
async def test_compile_async_exception(
    compiler_instance,
    mock_process_manager,
    mock_diagnostic_parser,
    mock_compiler_metrics,
    ...
    mocker.patch.object(
        ...
        "_build_compile_command",
        new_callable=AsyncMock,
        side_effect=Exception("Unexpected build error"),
    )
    ...
    mock_compiler_metrics.record_compilation.assert_called_once_with(
        False, mocker.ANY, is_link=False
    )  # Duration will be non-zero
...
def test_compile_sync(compiler_instance, mocker):
    ...
    mock_compile_async = mocker.patch.object(
        compiler_instance, "compile_async", new_callable=AsyncMock
    )
...
@pytest.mark.asyncio
async def test_link_async_success(
    compiler_instance,
    mock_process_manager,
    mock_diagnostic_parser,
    mock_compiler_metrics,
    ...
    result = await compiler_instance.link_async(
        object_files=object_files, output_file=output_file, options=options
    )
    ...
    compiler_instance._process_compilation_result.assert_called_once()
    mock_compiler_metrics.record_compilation.assert_called_once_with(
        True, 0.2, is_link=True
    )


@pytest.mark.asyncio
async def test_link_async_command_failure(
    compiler_instance,
    mock_process_manager,
    mock_diagnostic_parser,
    mock_compiler_metrics,
    ...
    compiler_instance._process_compilation_result.assert_called_once()
    mock_compiler_metrics.record_compilation.assert_called_once_with(
        False, 0.15, is_link=True
    )


@pytest.mark.asyncio
async def test_link_async_exception(
    compiler_instance,
    mock_process_manager,
    mock_diagnostic_parser,
    mock_compiler_metrics,
    ...
    mocker.patch.object(
        ...
        "_build_link_command",
        new_callable=AsyncMock,
        side_effect=Exception("Unexpected link error"),
    )
...
def test_link_sync(compiler_instance, mocker):
    ...
    mock_asyncio_run = mocker.patch("asyncio.run")
    # Mock the async method it calls
    mock_link_async = mocker.patch.object(
        compiler_instance, "link_async", new_callable=AsyncMock
    )
    ...
    mock_asyncio_run.assert_called_once()
    # Check that asyncio.run was called with the correct coroutine
    mock_link_async.assert_called_once_with(object_files, output_file, options, None)
...
async def test__build_compile_command_gcc(compiler_instance, tmp_path):
    ...
    assert (
        cmd[-1 - len(source_files) - 1 : -2]
        == expected_cmd_parts[-1 - len(source_files) - 1 : -2]
    )  # Source files
...
async def test__build_compile_command_clang(compiler_instance_clang, tmp_path):
    ...
    cpp_version = (
        CppVersion.CPP17
    )  # Still use C++ version flag even for C file in this context
    options = CompileOptions(
...
@pytest.mark.asyncio
async def test__build_compile_command_unsupported_cpp_version(
    compiler_instance, tmp_path
):
    ...
    assert set(excinfo.value.supported_versions) == {
        CppVersion.CPP17,
        CppVersion.CPP20,
        CppVersion.CPP23,
    }  # Based on mock_compiler_config_gcc
...
@pytest.mark.asyncio
async def test__build_link_command_gcc_shared_darwin(
    compiler_instance, tmp_path, mocker
):
    """Test building shared link command for GCC on Darwin (macOS)."""
    object_files = [tmp_path / "build" / "file1.o"]
    output_file = tmp_path / "libmylib.dylib"  # macOS shared lib extension
    options = LinkOptions(
        shared=True, runtime_library_paths=[tmp_path / "runtime_lib_mac"]
    )
...
async def test__build_link_command_msvc(compiler_instance_msvc, tmp_path):
    ...
    assert cmd[-3 - len(object_files) : -2] == [
        str(f) for f in object_files
    ]  # Object files before output
...
@pytest.mark.asyncio
async def test__process_compilation_result_success(
    compiler_instance, mock_diagnostic_parser, tmp_path
):
...
@pytest.mark.asyncio
async def test__process_compilation_result_failure_with_errors(
    compiler_instance, mock_diagnostic_parser, tmp_path
):
    ...
    assert result.notes == []
    mock_diagnostic_parser.parse_diagnostics.assert_called_once_with(
        "Error: syntax error"
    )


@pytest.mark.asyncio
async def test__process_compilation_result_failure_no_parsed_errors(
    compiler_instance, mock_diagnostic_parser, tmp_path
):
    ...
    cmd_result = CommandResult(
        returncode=1,
        stdout="",
        stderr="Some unexpected output on stderr",
        success=False,
    )
    command = ["mock_compiler", "main.cpp"]
    ...
    assert result.notes == []
    mock_diagnostic_parser.parse_diagnostics.assert_called_once_with(
        "Some unexpected output on stderr"
    )
...
async def test_get_version_info_async_gcc(compiler_instance, mock_process_manager):
    ...
    mock_process_manager.run_command_async.return_value = CommandResult(
        returncode=0,
        stdout="g++ (Ubuntu 11.3.0-1ubuntu1~22.04) 11.3.0\nCopyright (C) 2021 Free Software Foundation, Inc.",
        stderr="",
        success=True,
    )
...
@pytest.mark.asyncio
async def test_get_version_info_async_msvc(
    compiler_instance_msvc, mock_process_manager
):
    """Test getting version info for MSVC."""
    mock_process_manager.run_command_async.return_value = CommandResult(
        returncode=0,
        stdout="",
        stderr="Microsoft (R) C/C++ Optimizing Compiler Version 19.35.32215 for x64\n",
        success=True,
    )
    ...
    assert (
        "Version 19.35.32215" in version_info["full_output"]
    )  # MSVC puts output on stderr
    mock_process_manager.run_command_async.assert_called_once_with(
        ["C:\\VC\\cl.exe", "/Bv"]
    )
...
async def test_get_version_info_async_failure(compiler_instance, mock_process_manager):
    ...
    mock_process_manager.run_command_async.assert_called_once_with(
        ["/usr/bin/non_existent_compiler", "--version"]
    )
...
def test_get_version_info_sync(compiler_instance, mocker):
    ...
    mock_asyncio_run = mocker.patch("asyncio.run")
    mock_get_version_info_async = mocker.patch.object(
        compiler_instance, "get_version_info_async", new_callable=AsyncMock
    )
...
"""
    ...
    assert notes == ["/path/to/file1.cpp:11:6: note: in expansion of macro 'MY_MACRO'"]
...
"""
    ...
    assert notes == ["file1.cpp(11) : note: see expansion of macro 'MY_MACRO'"]
