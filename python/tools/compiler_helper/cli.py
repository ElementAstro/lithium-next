...
"""
    ...
    parser.add_argument("--compiler", type=str,
                        help="Compiler to use (GCC, Clang, MSVC)")
    parser.add_argument("--cpp-version", type=str, default="c++17",
                        help="C++ standard version (e.g., c++17, c++20)")
    parser.add_argument("--link", action="store_true",
                        help="Link the object files into an executable")
    ...
    compile_group.add_argument("--include-path", "-I", action="append",
                               dest="include_paths", help="Add include directory")
    compile_group.add_argument("--define", "-D", action="append",
                               dest="defines", help="Add preprocessor definition")
    compile_group.add_argument(
        "--warnings", "-W", action="append", help="Add warning flags")
    compile_group.add_argument(
        "--optimization", "-O", help="Set optimization level")
    compile_group.add_argument(
        "--debug", "-g", action="store_true", help="Include debug information")
    compile_group.add_argument(
        "--pic", action="store_true", help="Generate position-independent code")
    compile_group.add_argument(
        "--stdlib", help="Specify standard library to use")
    compile_group.add_argument(
        "--sanitize", action="append", dest="sanitizers", help="Enable sanitizer")

    # Linking options
    link_group = parser.add_argument_group("Linking options")
    link_group.add_argument("--library-path", "-L", action="append",
                            dest="library_paths", help="Add library directory")
    link_group.add_argument("--library", "-l", action="append",
                            dest="libraries", help="Add library to link against")
    link_group.add_argument("--shared", action="store_true",
                            help="Create a shared library")
    link_group.add_argument(
        "--static", action="store_true", help="Prefer static linking")
    link_group.add_argument(
        "--strip", action="store_true", help="Strip debug symbols")
    ...
    parser.add_argument("--config", type=Path,
                        help="Load options from configuration file (JSON)")
