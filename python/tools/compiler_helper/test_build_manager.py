...
from .core_types import (
    CompilationResult, CompileOptions, LinkOptions, CppVersion, PathLike
)
...
def mock_compiler(mock_compiler_config):
    ...
    async def mock_compile_async(source_files, output_file, cpp_version, options, timeout=None):
        # Simulate creating the output file
        Path(output_file).parent.mkdir(parents=True, exist_ok=True)
        Path(output_file).touch()
        return CompilationResult(success=True, output_file=Path(output_file), duration_ms=100, warnings=[], errors=[])
    ...
    async def mock_link_async(object_files, output_file, options, timeout=None):
        ...
        Path(output_file).parent.mkdir(parents=True, exist_ok=True)
        Path(output_file).touch()
        return CompilationResult(success=True, output_file=Path(output_file), duration_ms=200, warnings=[], errors=[])
...
@pytest.fixture
def build_manager(tmp_path, mock_compiler_manager, mock_file_manager, mock_process_manager):
    build_dir = tmp_path / "build"
    # Patch FileManager and ProcessManager in the BuildManager class for the fixture
    with patch('tools.compiler_helper.build_manager.FileManager', return_value=mock_file_manager), \
            patch('tools.compiler_helper.build_manager.ProcessManager', return_value=mock_process_manager):
...
def mock_calculate_file_hash_async(mocker):
    mock_hash = mocker.patch(
        'tools.compiler_helper.build_manager.BuildManager._calculate_file_hash_async', new_callable=AsyncMock)
...
def mock_scan_dependencies_async(mocker):
    mock_scan = mocker.patch(
        'tools.compiler_helper.build_manager.BuildManager._scan_dependencies_async', new_callable=AsyncMock)
...
@pytest.mark.asyncio
async def test_build_async_success(build_manager, mock_compiler, create_source_files, tmp_path):
    ...
    assert all(Path(obj).exists()
               for obj in linked_objects)  # Check if mock compile created them
...
@pytest.mark.asyncio
async def test_build_async_incremental_no_changes(build_manager, mock_compiler, create_source_files, tmp_path, mock_calculate_file_hash_async, mock_scan_dependencies_async):
    source_files = create_source_files(["src/file1.cpp", "src/file2.cpp"])
    output_file = tmp_path / "app"
...
@pytest.mark.asyncio
async def test_build_async_incremental_source_change(build_manager, mock_compiler, create_source_files, tmp_path, mock_calculate_file_hash_async, mock_scan_dependencies_async):
    source_files = create_source_files(["src/file1.cpp", "src/file2.cpp"])
    output_file = tmp_path / "app"
    ...
    cached_entry = build_manager.dependency_cache[str(
        source_files[0].resolve())]
    assert cached_entry.file_hash != hashlib.md5(
        b"int main() { return 0; }").hexdigest()
    assert cached_entry.file_hash == hashlib.md5(
        b"int main() { return 1; }").hexdigest()


@pytest.mark.asyncio
async def test_build_async_incremental_dependency_change(build_manager, mock_compiler, create_source_files, tmp_path, mock_calculate_file_hash_async, mock_scan_dependencies_async):
    header_file = create_source_files(
        ["include/header.h"], content="#define VERSION 1")[0]
    source_files = create_source_files(
        ["src/file1.cpp"], content=f'#include "include/header.h"\nint main() {{ return VERSION; }}')
    output_file = tmp_path / "app"
    ...
    assert str(header_file.resolve()
    ...
    # Check cache update
    assert len(build_manager.dependency_cache) == len(
        source_files) + 1  # Source + Header
    # The hash for the header file should be updated in the cache
    cached_header_entry = build_manager.dependency_cache[str(
        header_file.resolve())]
    assert cached_header_entry.file_hash == hashlib.md5(
        b"#define VERSION 2").hexdigest()


@pytest.mark.asyncio
async def test_build_async_force_rebuild(build_manager, mock_compiler, create_source_files, tmp_path):
    source_files = create_source_files(["src/file1.cpp", "src/file2.cpp"])
    output_file = tmp_path / "app"
...
@pytest.mark.asyncio
async def test_build_async_compilation_failure(build_manager, mock_compiler, create_source_files, tmp_path):
    source_files = create_source_files(["src/file1.cpp", "src/file2.cpp"])
    output_file = tmp_path / "app"

    # Configure mock compiler to fail compilation for one file
    async def mock_compile_fail(source_files_list, output_file, cpp_version, options, timeout=None):
        source_file = source_files_list[0]  # Assuming one file per call
        if "file1" in str(source_file):
            return CompilationResult(success=False, errors=["Mock compilation error"], warnings=[], duration_ms=50)
        else:
...
@pytest.mark.asyncio
async def test_build_async_linking_failure(build_manager, mock_compiler, create_source_files, tmp_path):
    source_files = create_source_files(["src/file1.cpp", "src/file2.cpp"])
    output_file = tmp_path / "app"
...
@pytest.mark.asyncio
async def test_build_async_parallel_compilation(build_manager, mock_compiler, create_source_files, tmp_path):
    source_files = create_source_files(
        [f"src/file{i}.cpp" for i in range(5)])  # More than 1 file
    output_file = tmp_path / "app"
...
@pytest.mark.asyncio
async def test_build_async_sequential_compilation(build_manager, mock_compiler, create_source_files, tmp_path):
...
@pytest.mark.asyncio
async def test_build_async_cache_disabled(build_manager, mock_compiler, create_source_files, tmp_path):
    source_files = create_source_files(["src/file1.cpp"])
    output_file = tmp_path / "app"
    ...
    assert not build_manager.cache_file.exists(
...
def test_load_cache_success(build_manager, tmp_path):
    ...
    file1_entry = build_manager.dependency_cache.get(
        str(tmp_path / "src/file1.cpp"))
