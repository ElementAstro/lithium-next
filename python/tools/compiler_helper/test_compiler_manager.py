...
def mock_system_info(mocker):
    mock_sys_info = mocker.patch('tools.compiler_helper.compiler_manager.SystemInfo', autospec=True)
    mock_sys_info.get_cpu_count.return_value = 4
    mock_sys_info.get_platform_info.return_value = {"system": "Linux", "release": "5.15"}
...
def mock_compiler_config(mock_compiler_config_data):
    ...
    config.additional_compile_flags = mock_compiler_config_data['additional_compile_flags']
    ...
    config.features.supported_cpp_versions = mock_compiler_config_data['features']['supported_cpp_versions']
    config.features.supported_sanitizers = mock_compiler_config_data['features']['supported_sanitizers']
    config.features.supported_optimizations = mock_compiler_config_data['features']['supported_optimizations']
    config.features.supports_parallel = mock_compiler_config_data['features']['supports_parallel']
    config.features.supports_pch = mock_compiler_config_data['features']['supports_pch']
    config.features.supports_modules = mock_compiler_config_data['features']['supports_modules']
    config.features.supports_concepts = mock_compiler_config_data['features']['supports_concepts']
    config.features.max_parallel_jobs = mock_compiler_config_data['features']['max_parallel_jobs']
    return config
...
@pytest.mark.asyncio
async def test_detect_compilers_async_found(compiler_manager, mock_compiler_class, mocker):
    # Mock shutil.which to simulate finding g++ and clang++
    mocker.patch('shutil.which', side_effect=lambda cmd: f'/usr/bin/{cmd}' if cmd in ['g++', 'clang++'] else None)
    # Mock _get_compiler_version_async and _create_compiler_features
    mocker.patch.object(compiler_manager, '_get_compiler_version_async', new_callable=AsyncMock, return_value='10.2.0')
    mocker.patch.object(compiler_manager, '_create_compiler_features', return_value=MagicMock(spec=CompilerFeatures))
    ...
    assert compiler_manager.default_compiler in ['GCC', 'Clang'] # Default should be one of the found
    mock_compiler_class.call_count == len(detected) # Compiler constructor called for each found


@pytest.mark.asyncio
async def test_detect_compilers_async_not_found(compiler_manager, mock_compiler_class, mocker):
...
@pytest.mark.asyncio
async def test_detect_compilers_async_partial_failure(compiler_manager, mock_compiler_class, mocker):
    # Mock shutil.which to find g++ but not clang++
    mocker.patch('shutil.which', side_effect=lambda cmd: '/usr/bin/g++' if cmd == 'g++' else None)
...
def test_detect_compilers_sync(compiler_manager, mock_compiler_class, mocker):
    ...
    mocker.patch.object(compiler_manager, '_get_compiler_version_async', return_value='10.2.0')
    mocker.patch.object(compiler_manager, '_create_compiler_features', return_value=MagicMock(spec=CompilerFeatures))
...
@pytest.mark.asyncio
async def test_get_compiler_async_by_name(compiler_manager, mock_compiler_instance, mock_compiler_class, mocker):
    ...
    mocker.patch.object(compiler_manager, 'detect_compilers_async', new_callable=AsyncMock)
    compiler_manager.detect_compilers_async.assert_not_called()


@pytest.mark.asyncio
async def test_get_compiler_async_default(compiler_manager, mock_compiler_instance, mock_compiler_class, mocker):
    # Simulate compilers being detected
    compiler_manager.compilers = {
        'GCC': mock_compiler_instance,
...
@pytest.mark.asyncio
async def test_get_compiler_async_detect_if_empty(compiler_manager, mock_compiler_class, mocker):
...
@pytest.mark.asyncio
async def test_get_compiler_async_not_found(compiler_manager, mock_compiler_class, mocker):
    # Simulate compilers being detected
    compiler_manager.compilers = {
...
@pytest.mark.asyncio
async def test_get_compiler_async_no_compilers_detected(compiler_manager, mock_compiler_class, mocker):
    # Ensure compilers are initially empty
    compiler_manager.compilers = {}
    compiler_manager.default_compiler = None
...
def test_get_compiler_sync_by_name(compiler_manager, mock_compiler_instance, mock_compiler_class, mocker):
    # Simulate compilers being detected
    compiler_manager.compilers = {
        'GCC': mock_compiler_instance,
        'Clang': MagicMock(spec=Compiler)
...
def test_get_compiler_sync_default(compiler_manager, mock_compiler_instance, mock_compiler_class, mocker):
    # Simulate compilers being detected
    compiler_manager.compilers = {
        'GCC': mock_compiler_instance,
        'Clang': MagicMock(spec=Compiler)
...
def test_get_compiler_sync_detect_if_empty(compiler_manager, mock_compiler_class, mocker):
    # Ensure compilers are initially empty
    compiler_manager.compilers = {}
    compiler_manager.default_compiler = None
...
def test_get_compiler_sync_no_compilers_detected(compiler_manager, mock_compiler_class, mocker):
    # Ensure compilers are initially empty
    compiler_manager.compilers = {}
    compiler_manager.default_compiler = None
...
@pytest.mark.asyncio
async def test__detect_compiler_async_success_path(compiler_manager, mock_compiler_class, mocker):
    spec = CompilerSpec(
        name="TestCompiler",
    ...
    mocker.patch.object(compiler_manager, '_get_compiler_version_async', new_callable=AsyncMock, return_value='1.0.0')
    mocker.patch.object(compiler_manager, '_create_compiler_features', return_value=MagicMock(spec=CompilerFeatures))
...
@pytest.mark.asyncio
async def test__detect_compiler_async_success_find_method(compiler_manager, mock_compiler_class, mocker):
    ...
    mocker.patch.object(compiler_manager, '_get_compiler_version_async', new_callable=AsyncMock, return_value='2.0.0')
    mocker.patch.object(compiler_manager, '_create_compiler_features', return_value=MagicMock(spec=CompilerFeatures))
...
@pytest.mark.asyncio
async def test__detect_compiler_async_not_found(compiler_manager, mock_compiler_class, mocker):
    spec = CompilerSpec(
...
@pytest.mark.asyncio
async def test__detect_compiler_async_compiler_config_validation_error(compiler_manager, mock_compiler_class, mocker):
    spec = CompilerSpec(
    ...
    mocker.patch('tools.compiler_helper.compiler_manager.CompilerConfig', side_effect=ValidationError([], MagicMock()))
...
@pytest.mark.asyncio
async def test__detect_compiler_async_compiler_exception(compiler_manager, mock_compiler_class, mocker):
    spec = CompilerSpec(
...
async def test__get_compiler_version_async_gcc_clang(compiler_manager, mocker):
    ...
    mock_process.communicate.return_value = (b"GCC version 11.3.0 (Ubuntu 11.3.0-1ubuntu1~22.04)\n", b"")
    mocker.patch('asyncio.create_subprocess_exec', new_callable=AsyncMock, return_value=mock_process)

    version = await compiler_manager._get_compiler_version_async("/usr/bin/g++", CompilerType.GCC)
    assert version == "11.3.0"
    asyncio.create_subprocess_exec.assert_called_once_with("/usr/bin/g++", "--version", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)

    mock_process.communicate.return_value = (b"clang version 14.0.0\n", b"")
    asyncio.create_subprocess_exec.reset_mock()
    version = await compiler_manager._get_compiler_version_async("/usr/bin/clang++", CompilerType.CLANG)
    assert version == "14.0.0"
    asyncio.create_subprocess_exec.assert_called_once_with("/usr/bin/clang++", "--version", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)


@pytest.mark.asyncio
async def test__get_compiler_version_async_msvc(compiler_manager, mocker):
    mock_process = AsyncMock()
    mock_process.communicate.return_value = (b"", b"Microsoft (R) C/C++ Optimizing Compiler Version 19.35.32215 for x64\n")
    mocker.patch('asyncio.create_subprocess_exec', new_callable=AsyncMock, return_value=mock_process)

    version = await compiler_manager._get_compiler_version_async("C:\\Program Files\\VC\\Tools\\cl.exe", CompilerType.MSVC)
    assert version == "19.35.32215"
    asyncio.create_subprocess_exec.assert_called_once_with("C:\\Program Files\\VC\\Tools\\cl.exe", stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
...
async def test__get_compiler_version_async_unknown(compiler_manager, mocker):
    ...
    version = await compiler_manager._get_compiler_version_async("/usr/bin/unknown_compiler", CompilerType.GCC)
...
def test__create_compiler_features_clang(compiler_manager):
    ...
    features_16 = compiler_manager._create_compiler_features(CompilerType.CLANG, "16.0.0")
...
def test__create_compiler_features_msvc(compiler_manager):
    features = compiler_manager._create_compiler_features(CompilerType.MSVC, "19.28.29910")
    assert CppVersion.CPP17 in features.supported_cpp_versions
    assert CppVersion.CPP20 in features.supported_cpp_versions
    ...
    assert OptimizationLevel.FAST not in features.supported_optimizations # MSVC doesn't have Ofast
    assert features.supports_modules is False # MSVC < 19.29
    assert features.supports_concepts is False # MSVC < 19.30

    features_19_30 = compiler_manager._create_compiler_features(CompilerType.MSVC, "19.30.30704")
...
def test__find_msvc_windows_vswhere_success(compiler_manager, mocker, tmp_path):
    ...
    mocker.patch('os.environ.get', return_value=str(tmp_path.parent)) # Mock ProgramFiles(x86)
    mocker.patch('pathlib.Path.__new__', side_effect=lambda cls, *args: Path(os.path.join(*args)) if args[0] != str(tmp_path.parent) else mock_vswhere_path) # Mock Path constructor for vswhere path
    ...
    mocker.patch.object(Path, 'iterdir', return_value=[mock_version_path], autospec=True)
    mocker.patch.object(Path, 'is_dir', return_value=True, autospec=True) # For iterdir results
...
def test__find_msvc_windows_vswhere_not_found(compiler_manager, mocker, tmp_path):
    ...
    mocker.patch('pathlib.Path.__new__', side_effect=lambda cls, *args: Path(os.path.join(*args)) if args[0] != str(tmp_path.parent) else tmp_path / "non_existent_vswhere.exe")
...
def test__find_msvc_windows_vswhere_failure(compiler_manager, mocker, tmp_path):
    ...
    mock_vswhere_path.touch()
    mocker.patch('os.environ.get', return_value=str(tmp_path.parent))
    mocker.patch('pathlib.Path.__new__', side_effect=lambda cls, *args: Path(os.path.join(*args)) if args[0] != str(tmp_path.parent) else mock_vswhere_path)
...
def test_list_compilers(compiler_manager, mock_compiler_instance, mock_compiler_class, mocker):
    # Simulate compilers being detected
    compiler_manager.compilers = {
        'GCC': mock_compiler_instance,
        'Clang': MagicMock(spec=Compiler)
    ...
    compiler_manager.compilers['Clang'].config.features = MagicMock(spec=CompilerFeatures)
    compiler_manager.compilers['Clang'].config.features.supported_cpp_versions = {CppVersion.CPP17, CppVersion.CPP20}
