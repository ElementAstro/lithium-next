...
from typing import (
    List, Dict, Optional, Union, Tuple, Any, AsyncIterator,
    Literal, overload, TypeGuard, Self
)
...
from .exceptions import (
    ...
    GitRebaseConflictError, GitCherryPickError, GitRemoteError,
    GitTagError, GitStashError, GitConfigError, GitRepositoryNotFound,
    create_git_error_context
)
from .models import (
    GitResult, CommitInfo, StatusInfo, FileStatus, AheadBehindInfo,
    BranchInfo, RemoteInfo, TagInfo, GitOperation, GitStatusCode,
    BranchType, ResetMode, MergeStrategy, GitOutputFormat,
    CommitSHA, BranchName, TagName, RemoteName, FilePath, CommitMessage
)
from .utils import (
    change_directory, ensure_path, validate_repository,
...
class GitUtils:
    ...
    def add_changes(
        self, 
        paths: Optional[Union[str, List[str], Path, List[Path]]] = None
    ) -> GitResult:
        """
    ...
    def commit_changes(
        ...
        with change_directory(self.repo_dir):
            result = self.run_git_command(command)
            if result.success:
                logger.info(f"Successfully committed changes: {sanitized_message[:50]}...")
            return result
    ...
    def parse_status(self, output: str) -> List[FileStatus]:
        ...
        for line in output.strip().split('\n'): # Changed from '\\n' to '\n'
            ...
            x_status = GitStatusCode(line[0]) if line[0] in [s.value for s in GitStatusCode] else GitStatusCode.UNMODIFIED
            y_status = GitStatusCode(line[1]) if line[1] in [s.value for s in GitStatusCode] else GitStatusCode.UNMODIFIED
            path = line[3:]
            ...
            if x_status in [GitStatusCode.RENAMED, GitStatusCode.COPIED] and '->' in path:
                parts = path.split(' -> ')
                if len(parts) == 2:
        ...
        try:
            ...
            if result.success and result.output.strip():
                try:
                    ...
                    logger.debug(f"Failed to parse ahead/behind output: {result.output}")
