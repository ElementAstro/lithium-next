...
def test_verify_checksum_mismatch(algorithm: ChecksumAlgo):
    ...
    wrong_checksum = "a" * len(
        CORRECT_CHECKSUMS[algorithm]
    )  # Create a checksum of the correct length but wrong value
    if (
        wrong_checksum == CORRECT_CHECKSUMS[algorithm]
    ):  # Handle edge case if the above creates the correct one
...
def test_verify_checksum_invalid_expected_checksum_type():
    ...
    assert excinfo.value.context["algorithm"] == ChecksumAlgo.MD5
    assert (
        excinfo.value.context["expected_checksum"] == "12345"
    )  # Should be converted to string in context
...
def test_verify_checksum_generate_checksum_error(mock_generate_checksum):
    ...
    assert (
        excinfo.value.error_code == "MOCK_GENERATION_FAILED"
    )  # ChecksumError propagates
...
def test_verify_checksum_unexpected_error_during_generation(mock_generate_checksum):
    ...
    assert (
        excinfo.value.error_code == "VERIFICATION_FAILED"
    )  # verify_checksum catches and wraps
    assert "Failed to verify SHA256 checksum: Unexpected error during hash" in str(
        excinfo.value
    )
    assert excinfo.value.context["algorithm"] == ChecksumAlgo.SHA256
    assert excinfo.value.context["expected_checksum"] == "some_checksum"
...
def test_verify_checksum_unsupported_algorithm(mock_get_calculator):
    ...
    mock_get_calculator.side_effect = ChecksumError(
        "Unsupported checksum algorithm: fake_algo", algorithm="fake_algo"
    )
    ...
    assert (
        excinfo.value.error_code == "UNSUPPORTED_ALGORITHM"
    )  # Error from _get_checksum_calculator propagates
