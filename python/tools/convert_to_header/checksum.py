...
def _get_checksum_calculator(algorithm: ChecksumAlgo) -> ChecksumProtocol:
    ...
    if algorithm not in calculators:
        raise ChecksumError(
            f"Unsupported checksum algorithm: {algorithm}", algorithm=algorithm
        )
...
def generate_checksum(data: bytes, algorithm: ChecksumAlgo) -> str:
    ...
    if not isinstance(data, bytes):
        raise ChecksumError(
            f"Data must be bytes, got {type(data).__name__}", algorithm=algorithm
        )
    ...
    try:
        ...
        raise ChecksumError(
            f"Failed to generate {algorithm} checksum: {e}", algorithm=algorithm
        ) from e


def verify_checksum(
    data: bytes, expected_checksum: str, algorithm: ChecksumAlgo
) -> bool:
    """
...
def is_valid_checksum_format(checksum: str, algorithm: ChecksumAlgo) -> bool:
    ...
    expected_lengths = {"md5": 32, "sha1": 40, "sha256": 64, "sha512": 128, "crc32": 8}
    ...
    return len(checksum) == expected_length and all(
