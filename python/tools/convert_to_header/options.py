...
class ConversionOptions:
    ...
    def from_dict(cls, options_dict: dict[str, Any]) -> ConversionOptions:
        ...
        try:
            ...
            raise ValidationError(
                f"Failed to create options from dictionary: {e}"
            ) from e
    ...
    def from_json(cls, json_file: PathLike) -> ConversionOptions:
        ...
        try:
            ...
            raise ValidationError(
                f"Invalid JSON in configuration file: {e}", file_path=json_path
            ) from e
        except (OSError, IOError) as e:
            raise ValidationError(
                f"Failed to read configuration file: {e}", file_path=json_path
            ) from e
    ...
    def from_yaml(cls, yaml_file: PathLike) -> ConversionOptions:
        ...
        try:
            ...
            raise ValidationError(
                f"Invalid YAML in configuration file: {e}", file_path=yaml_path
            ) from e
        except (OSError, IOError) as e:
            raise ValidationError(
                f"Failed to read configuration file: {e}", file_path=yaml_path
            ) from e
    ...
    def save_to_json(self, json_file: PathLike) -> None:
        ...
        try:
            ...
            raise ValidationError(
                f"Failed to save configuration file: {e}", file_path=json_path
            ) from e
