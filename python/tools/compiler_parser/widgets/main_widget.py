...
class CompilerParserWidget:
    ...
    def parse_from_files(
        ...
        if combine_outputs:
            ...
            return combined if combined else CompilerOutput(
                compiler=CompilerType.from_string(compiler_type) if isinstance(compiler_type, str) else compiler_type,
                version="unknown"
            )
    ...
    def display_output(self, compiler_output: CompilerOutput, colorize: bool = True) -> None:
