...
Examples:
  ...
  wifi-hotspot config save --file /path/to/config.json
    ...
    def _add_start_parser(self, subparsers: Any) -> None:
        ...
        parser.add_argument(
            "--name", "--ssid",
            help="SSID (network name) for the hotspot"
        )
        parser.add_argument(
            "--password",
            help="Password for securing the hotspot (8-63 characters)"
        )
        parser.add_argument(
            "--interface",
            help="Network interface to use (e.g., wlan0)"
        )
        parser.add_argument(
            "--auth", "--authentication",
    ...
    def _add_clients_parser(self, subparsers: Any) -> None:
        ...
        parser.add_argument(
            "--monitor",
            action="store_true",
            help="Monitor clients in real-time"
        )
        parser.add_argument(
            "--interval",
            type=int,
...
class HotspotCLI:
    ...
    async def handle_config(self, args: argparse.Namespace) -> None:
        ...
        if args.config_action == "save":
            ...
            self.output.console.print(f"[green]✓[/green] Configuration saved to {args.file}")
            ...
            try:
                ...
                self.output.console.print(f"[green]✓[/green] Configuration loaded from {args.file}")
            except Exception as e:
            ...
            try:
                config = HotspotConfig.from_file(args.file)
                self.output.console.print(f"[green]✓[/green] Configuration file {args.file} is valid")
