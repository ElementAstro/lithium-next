...
from typing import (
    Any, AsyncContextManager, AsyncGenerator, AsyncIterator, Awaitable, Callable, Dict, 
    List, Optional, Protocol, Union
)
...
class HotspotManager:
    ...
    async def _ensure_network_manager(self) -> None:
        ...
        try:
            ...
            raise NetworkManagerError(
                "NetworkManager command timed out",
                error_code="NM_TIMEOUT"
            ) from None
    ...
    async def validate_interface(self, interface: str) -> bool:
        ...
        target_interface = next(
            (iface for iface in interfaces if iface.name == interface),
            None
        )
    ...
    async def get_connected_clients(
        ...
        try:
            ...
            if not iw_result.success:
                logger.debug(f"Failed to get station dump for {interface}: {iw_result.stderr}")
                return []
    ...
    async def _apply_advanced_config(self, cfg: HotspotConfig) -> None:
        ...
        for cmd in commands:
            result = await self.run_command(cmd)
            if not result.success:
                logger.warning(f"Failed to apply config: {' '.join(cmd)}: {result.stderr}")
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise HotspotException(
                f"Failed to get hotspot status: {e}",
    ...
    async def managed_hotspot(
        ...
        **config_overrides: Any
        ...
        try:
            ...
            if not success:
                raise HotspotException(
                    "Failed to start managed hotspot",
                    error_code="MANAGED_START_FAILED"
                )
        ...
        try:
            while True:
                ...
                disconnected_clients = [
                    seen_clients[mac] for mac in disconnected_macs
                ]
                ...
                # Update seen clients
                seen_clients = {client.mac_address: client for client in current_clients}
