...
class BandType(StrEnum):
    ...
    def frequency_ghz(self) -> str:
        ...
        return {self.G_ONLY: "2.4", self.A_ONLY: "5.0", self.DUAL: "2.4/5.0"}[self]
...
class HotspotConfig(BaseModel):
    ...
    encryption: EncryptionType = Field(
        default=EncryptionType.AES, description="Encryption algorithm for the hotspot"
    )
    ...
    max_clients: int = Field(
        default=10, ge=1, le=50, description="Maximum number of concurrent clients"
    )
    ...
    band: BandType = Field(
        default=BandType.G_ONLY, description="Frequency band to use for the hotspot"
    )

    hidden: bool = Field(default=False, description="Whether to hide the network SSID")
...
class ConnectedClient(BaseModel):
    ...
    hostname: Optional[str] = Field(
        default=None, max_length=253, description="Hostname of the connected client"
    )

    connected_since: Optional[float] = Field(
        default=None, description="Timestamp when client connected"
    )

    data_transferred: int = Field(
        default=0, ge=0, description="Total bytes transferred by this client"
    )

    signal_strength: Optional[int] = Field(
        default=None, ge=-100, le=0, description="Signal strength in dBm"
    )
