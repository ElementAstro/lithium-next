...
def parse_arguments():
    ...
    basic_group.add_argument(
        "--update-db", action="store_true", help="Update the package database"
    )
    basic_group.add_argument(
        "--upgrade", action="store_true", help="Upgrade the system"
    )
    basic_group.add_argument(
        "--install", type=str, metavar="PACKAGE", help="Install a package"
    )
    basic_group.add_argument(
        "--install-multiple",
        type=str,
    ...
    basic_group.add_argument(
        "--remove", type=str, metavar="PACKAGE", help="Remove a package"
    )
    basic_group.add_argument(
        "--remove-deps",
        action="store_true",
    ...
    basic_group.add_argument(
        "--search", type=str, metavar="QUERY", help="Search for a package"
    )
    basic_group.add_argument(
    ...
    adv_group.add_argument(
        "--list-outdated", action="store_true", help="List outdated packages"
    )
    adv_group.add_argument(
        "--clear-cache", action="store_true", help="Clear package cache"
    )
    adv_group.add_argument(
        "--keep-recent",
        action="store_true",
    ...
    adv_group.add_argument(
        "--list-cache", action="store_true", help="List packages in local cache"
    )
    ...
    config_group.add_argument(
        "--list-group", type=str, metavar="GROUP", help="List all packages in a group"
    )
    config_group.add_argument(
        "--optional-deps",
        type=str,
    ...
    config_group.add_argument(
        "--enable-color", action="store_true", help="Enable color output in pacman"
    )
    config_group.add_argument(
        "--disable-color", action="store_true", help="Disable color output in pacman"
    )

    # AUR support
    aur_group = parser.add_argument_group("AUR Support")
    aur_group.add_argument(
        "--aur-install",
        type=str,
        metavar="PACKAGE",
    ...
    aur_group.add_argument(
        "--aur-search", type=str, metavar="QUERY", help="Search for packages in the AUR"
    )
    ...
    maint_group.add_argument(
        "--clean-orphaned", action="store_true", help="Remove orphaned packages"
    )
    maint_group.add_argument(
        "--export-packages",
        type=str,
...
def main():
    ...
    try:
        if args.update_db:
            ...
            deps, opt_deps = pacman.show_package_dependencies(args.show_dependencies)
            if json_output:
                print(
                    json.dumps(
                        {"dependencies": deps, "optional_dependencies": opt_deps}
                    )
                )
            else:
                print("Dependencies:")
            ...
            if json_output:
                ...
                if owner:
                    print(f"'{args.find_file_owner}' is owned by package: {owner}")
                else:
            ...
            if json_output:
                print(json.dumps({"success": success, "threads": args.multithread}))
            else:
                if success:
                    print(
            ...
            if json_output:
                ...
                print(
                    ...
                    if success
                    else "Failed to enable color output"
                )
            ...
            if json_output:
                ...
                print(
                    ...
                    if success
                    else "Failed to disable color output"
                )
            ...
            result = pacman.install_aur_package(args.aur_install, no_confirm=no_confirm)
            if json_output:
                print(json.dumps(result))
            else:
                print(result["stdout"] if result["success"] else result["stderr"])
            ...
            if json_output:
                print(json.dumps({"success": success, "file": args.export_packages}))
            else:
                if success:
                    print(f"Package list exported to {args.export_packages}")
                else:
                    print(f"Failed to export package list to {args.export_packages}")
            ...
            if json_output:
                print(json.dumps({"success": success, "file": args.import_packages}))
            else:
                if success:
                    print(f"Packages imported from {args.import_packages}")
                else:
                    print(f"Failed to import packages from {args.import_packages}")
