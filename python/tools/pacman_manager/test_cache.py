...
class MockPackageInfo(PackageInfo):
    def to_dict(self):
        return {
            ...
            "dependencies": [str(d.name) for d in self.dependencies] if self.dependencies else None,
        }
    ...
    def from_dict(cls, data):
        ...
        return cls(
            ...
            dependencies=[Dependency(name=PackageName(d)) for d in data["dependencies"]] if data.get("dependencies") else None,
        )
...
class TestPackageCache:
    ...
    def test_get_package_memory_hit(self, package_cache, mock_package_info):
        ...
        package_cache._memory_cache.put(f"package:{mock_package_info.name}", mock_package_info)
    ...
    def test_get_package_disk_hit(self, package_cache, mock_package_info):
        ...
        package_cache._put_to_disk(f"package:{mock_package_info.name}", mock_package_info)
        ...
        assert package_cache._memory_cache.get(f"package:{mock_package_info.name}") == retrieved_package
    ...
    def test_put_package(self, package_cache, mock_package_info):
        ...
        assert package_cache._memory_cache.get(f"package:{mock_package_info.name}") == mock_package_info

        # Verify on disk
        cache_file = package_cache.cache_dir / package_cache._safe_filename(f"package:{mock_package_info.name}.cache")
    ...
    def test_clear_all(self, package_cache, mock_package_info):
        ...
        package_cache.put_package(mock_package_info)
        package_cache.put_package(MockPackageInfo(name=PackageName("another-pkg"), version="1.0.0", repository="extra", installed=True, status=MockPackageInfo.PackageStatus.INSTALLED))
    ...
    def test_cleanup_expired_memory(self, package_cache, mock_package_info):
        ...
        package_cache._memory_cache.put(f"package:{mock_package_info.name}", mock_package_info, ttl=-1)
        package_cache._memory_cache.put("package:valid-pkg", mock_package_info, ttl=100) # Valid entry
    ...
    def test_cleanup_expired_disk(self, package_cache, mock_package_info):
        ...
        expired_file = package_cache.cache_dir / package_cache._safe_filename(f"{expired_key}.cache")
        ...
        assert (package_cache.cache_dir / package_cache._safe_filename(f"{valid_key}.cache")).exists()
    ...
    def test_safe_filename(self, package_cache):
        ...
        assert package_cache._safe_filename("package:name/with:slash\\colon") == "package_name_with_slash_colon"
        long_key = "a" * 200
    ...
    def test_load_from_disk_on_startup(self, temp_cache_dir, mock_package_info):
        ...
        with open(temp_cache_dir / package_cache._safe_filename(f"{valid_key}.cache"), 'wb') as f:
        ...
        with open(temp_cache_dir / package_cache._safe_filename(f"{expired_key}.cache"), 'wb') as f:
            pickle.dump(expired_data, f)

        # Corrupted entry
        with open(temp_cache_dir / package_cache._safe_filename(f"{corrupted_key}.cache"), 'w') as f:
            f.write("this is not a pickle")

        # Initialize PackageCache, which should load from disk
        cache = PackageCache({'use_disk_cache': True, 'cache_directory': str(temp_cache_dir)})

        assert cache._memory_cache.size == 1
        assert cache._memory_cache.get(valid_key) is not None
        assert cache._memory_cache.get(expired_key) is None # Expired should not be loaded

        # Verify expired and corrupted files are removed from disk
        assert not (temp_cache_dir / package_cache._safe_filename(f"{expired_key}.cache")).exists()
        assert not (temp_cache_dir / package_cache._safe_filename(f"{corrupted_key}.cache")).exists()
        assert (temp_cache_dir / package_cache._safe_filename(f"{valid_key}.cache")).exists()
