...
from typing import Dict, List, Optional, Tuple, Set, Any, Union, Protocol, runtime_checkable
...
from .exceptions import (
    CommandError, PackageNotFoundError, ConfigError, DependencyError,
    PermissionError, NetworkError, ValidationError, create_error_context, PacmanError, OperationError
)
...
from .pacman_types import (
    PackageName, PackageVersion, RepositoryName, CommandOutput,
    OperationResult, ManagerConfig
)
...
class PacmanManager:
    ...
    def _merge_configs(self, default: ManagerConfig, override: ManagerConfig) -> ManagerConfig:
    ...
    def _init_plugin_system(self) -> None:
        ...
        try:
            ...
            logger.debug(f"Plugin system initialized with {len(plugin_dirs)} directories")
        except ImportError:
    ...
    def _error_context(self, operation: str, **extra: Any) -> Generator[None, None, None]:
        ...
        try:
            logger.debug(f"Starting operation: {operation}", extra={"operation_id": operation_id, **extra})
            yield
    ...
    def _validate_package_name(self, package_name: str) -> PackageName:
        ...
        validations = [
            ...
            (r'^[a-zA-Z0-9][a-zA-Z0-9._+-]*$', "Package name contains invalid characters"),
    ...
    def run_command(
        ...
        """
        timeout = timeout or self._config.get('retry_config', {}).get('timeout_seconds', 300)
        ...
        with self._error_context("run_command", command=command):
            ...
            try:
                ...
                if process.returncode != 0:
                    ...
                    if process.returncode == 1 and "permission denied" in (process.stderr or "").lower():
                        raise PermissionError(
                            f"Insufficient permissions for command: {' '.join(final_command)}",
                            required_permission="sudo" if self._config.get('use_sudo') else "admin",
    ...
    def _prepare_command(self, command: List[str]) -> List[str]:
        ...
        if self._system_info.is_windows:
            ...
            if (use_sudo and 
                final_command[0] != 'sudo' and 
                os.geteuid() != 0 and
                final_command[0] in ['pacman', str(self._pacman_command)]):
                ...
                if line.startswith(' '):  # Description line
                    ...
                    if package_match:
                        ...
                        current_package = PackageInfo(
                            ...
                            status=PackageStatus.INSTALLED if status == 'installed' 
    ...
    def list_installed_packages(self, refresh: bool = False) -> dict[str, PackageInfo]:
        ...
        cache_valid = (
            not refresh and 
            bool(self._installed_packages_cache) and 
    ...
    def _parse_installed_packages_output(self, output: str) -> dict[str, PackageInfo]:
        ...
        try:
                    ...
                    match line.split(':', 1):
                        case ['Name', name_value]:
                            ...
                            if name:
                                ...
                                parse_errors.append(f"Empty package name at line {line_num}")
                        ...
                        case ['Depends On', deps_value] if current_package:
                            ...
                            if deps and deps.lower() != 'none':
                                ...
                                for dep in deps.split():
                                    dep = dep.strip()
                                    if dep:
                                        dep_list.append(Dependency(name=PackageName(dep)))
                        ...
                        case [field_name, _]:
                            ...
                            logger.trace(f"Unhandled field '{field_name}' at line {line_num}")
            ...
            # Return partial results instead of failing completely
            if packages:
                logger.info(f"Returning partial results: {len(packages)} packages parsed")
            ...
            try:
                ...
                for line in str(stdout).strip().split("\n"):
                    ...
                    if len(parts) >= 3:
                        ...
                        latest_version = str(parts[-1]) if len(parts) >= 4 else str(parts[2])
