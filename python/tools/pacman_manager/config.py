...
class PacmanConfig:
    ...
    def _parse_config(self) -> PacmanConfigState:
        ...
        if not self._state.is_dirty() and (self._state.options.options or self._state.repositories):
                ...
                for line_num, line in enumerate(f, 1):
                    ...
                    if '=' in line:
                        ...
                        if current_section == "options":
                            ...
                            logger.warning(f"Orphaned option '{key}' at line {line_num}")
    ...
    def set_option(self, option: str, value: str, create_backup: bool = True) -> bool:
        ...
        try:
            ...
            option_pattern = re.compile(rf'^#?\s*{re.escape(option)}\s*=.*$', re.MULTILINE)
            option_found = False
            ...
            if not option_found:
                ...
                for i, line in enumerate(lines):
                    if line.strip() == '[options]':
                        ...
                        option_added = True
                        logger.debug(f"Added new option '{option}' after [options] section")
                        break
    ...
    def get_enabled_repos(self) -> list[str]:
        ...
        try:
            config = self._parse_config()
            enabled_repos = [
                name for name, repo in config.repositories.items()
                if repo.enabled
            ]
    ...
    def validate_configuration(self) -> list[str]:
        ...
        try:
            ...
            if arch and arch not in ["auto", "i686", "x86_64", "armv6h", "armv7h", "aarch64"]:
