...
class NginxManager:
    ...
    def _print_color(self, message: str, color: OutputColors = OutputColors.RESET) -> None:
    ...
    async def _run_command(
        ...
        try:
            ...
            raise OperationError(f"Command '{command_str}' timed out", details={"timeout": str(e)}) from e
        except OSError as e:
            logger.error(f"OS error running command '{command_str}': {e}")
            raise OperationError(f"OS error: {e}", details={"command": command_str}) from e
        except Exception as e:
            logger.error(f"Unexpected error running command '{command_str}': {e}")
            raise OperationError(f"Unexpected error: {e}", details={"command": command_str}) from e
                ...
                case OperatingSystem.LINUX:
                    if Path("/etc/debian_version").exists():
                        ...
                        raise InstallationError(
                            ...
                            details={"detected_files": str(list(Path("/etc").glob("*-release")))}
                        )
                case OperatingSystem.MACOS:
    ...
    async def manage_service(self, action: str) -> None:
        ...
        if action not in valid_actions:
            raise ValueError(f"Invalid service action: {action}. Valid actions: {valid_actions}")
        ...
        try:
            ...
            if action in ("start", "restart") and not self.paths.binary_path.exists():
                raise OperationError(
                    ...
                    details={"binary_path": str(self.paths.binary_path), "action": action}
                )
            ...
            if isinstance(e, (OperationError, ValueError)):
                raise
            raise OperationError(f"Unexpected error during {action}: {e}", details={"action": action}) from e
    ...
    async def check_config(self) -> bool:
        ...
        try:
            ...
            self._print_color(f"Nginx configuration is invalid: {error_details}", OutputColors.RED)
    ...
    async def backup_config(self, custom_name: Optional[str] = None) -> Path:
        ...
        try:
            ...
            if backup_file.exists() and not custom_name:
                backup_file = self.paths.backup_path / f"nginx.conf.{timestamp}_{id(self)}.bak"
            ...
            raise ConfigError(
                ...
                details={"source": str(self.paths.conf_path), "backup_dir": str(self.paths.backup_path)}
            ) from e
        except Exception as e:
    ...
    async def restore_config(
        ...
        try:
            ...
            if not to_restore.exists():
                raise OperationError(
                    ...
                    details={
                        ...
                        "available_backups": [str(b) for b in backups[:5]]  # Show first 5
                    }
                )
            ...
            pre_restore_name = f"pre-restore-{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.bak"
            ...
            raise OperationError(
                ...
                details={"backup_file": str(to_restore) if 'to_restore' in locals() else "unknown"}
            ) from e
        except Exception as e:
            if isinstance(e, OperationError):
                raise
    ...
    async def _create_vhost(
        ...
        try:
            ...
            self._print_color(f"Virtual host '{server_name}' created.", OutputColors.GREEN)
            ...
            raise ConfigError(
                ...
                details={"server_name": server_name, "config_path": str(config_file) if 'config_file' in locals() else "unknown"}
            ) from e
    ...
    async def _enable_vhost(self, server_name: str, **_) -> None:
        ...
        try:
            ...
            if not source.exists():
                raise ConfigError(
                    ...
                    details={
                        ...
                        "available_configs": [f.stem for f in self.paths.sites_available.glob("*.conf")]
                    }
                )
            ...
            self._print_color(f"Virtual host '{server_name}' enabled.", OutputColors.GREEN)
            ...
            raise ConfigError(
                ...
                details={"server_name": server_name, "source": str(source), "target": str(target)}
            ) from e
    ...
    async def _disable_vhost(self, server_name: str, **_) -> None:
        ...
        try:
            ...
            if target.exists():
                target.unlink()
                self._print_color(f"Virtual host '{server_name}' disabled.", OutputColors.GREEN)
                logger.success(f"Virtual host disabled: {server_name}")
            else:
                self._print_color(f"Virtual host '{server_name}' is already disabled.", OutputColors.YELLOW)
    ...
    def list_virtual_hosts(self) -> dict[str, bool]:
        ...
        try:
            ...
            logger.debug(f"Found {len(available)} virtual hosts, {len(enabled)} enabled")
            return result
        ...
        if results["installed"]:
            ...
            try:
                ...
                if results["virtual_hosts"] > 0:
                    results["virtual_hosts_list"] = list(vhosts.keys())[:10]  # Limit to first 10
    ...
    def _print_health_results(self, results: dict[str, Any]) -> None:
        ...
        status_items = [
            ("Installed", results["installed"], OutputColors.GREEN if results["installed"] else OutputColors.RED),
            ("Running", results["running"], OutputColors.GREEN if results["running"] else OutputColors.RED),
            ("Config Valid", results["config_valid"], OutputColors.GREEN if results["config_valid"] else OutputColors.RED),
        ]
        ...
        if results["virtual_hosts"] > 0:
            ...
            self._print_color(f"  Virtual Hosts: {results['virtual_hosts']} total, {enabled} enabled", OutputColors.BLUE)
        ...
        overall_color = OutputColors.GREEN if results.get("healthy", False) else OutputColors.RED
