...
private:
     ...
     */
    static auto handleTaskAction(const crow::request& req,
    ...
    void registerRoutes(lithium::server::ServerApp& app) override {
        ...
        CROW_ROUTE(app, "/api/tasks/camera")
            .methods("POST"_method)([](const crow::request& req) {
                ...
                return handleTaskAction(
                    req, body, "createCameraTask", [&body]() -> nlohmann::json {
                        nlohmann::json result;

                        if (!body.has("taskType")) {
                            throw std::invalid_argument(
                                "Missing required parameter: taskType");
                        }
                        ...
                        // Convert crow::json to nlohmann::json for easier
                        // parameter handling
                        ...
                        if (body.has("binning"))
                            params["binning"] = body["binning"].i();
                        if (body.has("gain"))
                            params["gain"] = body["gain"].d();
                        if (body.has("offset"))
                            params["offset"] = body["offset"].i();
                        if (body.has("camera"))
                            params["camera"] = body["camera"].s();
                        if (body.has("x"))
                            params["x"] = body["x"].i();
                        if (body.has("y"))
                            params["y"] = body["y"].i();
                        ...
                        if (body.has("temperature"))
                            params["temperature"] = body["temperature"].d();
                        if (body.has("cooler"))
                            params["cooler"] = body["cooler"].b();
                        if (body.has("delay"))
                            params["delay"] = body["delay"].d();
                        if (body.has("step_size"))
                            params["step_size"] = body["step_size"].i();
                        if (body.has("max_steps"))
                            params["max_steps"] = body["max_steps"].i();
                        if (body.has("focuser"))
                            params["focuser"] = body["focuser"].s();
                        if (body.has("filter_wheel"))
                            params["filter_wheel"] = body["filter_wheel"].s();
                        if (body.has("guide_camera"))
                            params["guide_camera"] = body["guide_camera"].s();
                        if (body.has("guide_exposure"))
                            params["guide_exposure"] =
                                body["guide_exposure"].d();
                        if (body.has("settle_time"))
                            params["settle_time"] = body["settle_time"].d();
                        if (body.has("dark_count"))
                            params["dark_count"] = body["dark_count"].i();
                        if (body.has("bias_count"))
                            params["bias_count"] = body["bias_count"].i();
                        if (body.has("flat_count"))
                            params["flat_count"] = body["flat_count"].i();
                        if (body.has("dark_exposure"))
                            params["dark_exposure"] = body["dark_exposure"].d();
                        if (body.has("r_exposure"))
                            params["r_exposure"] = body["r_exposure"].d();
                        if (body.has("g_exposure"))
                            params["g_exposure"] = body["g_exposure"].d();
                        if (body.has("b_exposure"))
                            params["b_exposure"] = body["b_exposure"].d();
                        ...
                        std::string id = task_manager_->submitTask(
                            taskType, params, [taskType](const auto& info) {
                                auto& factory =
                                    lithium::task::TaskFactory::getInstance();
                                std::string factoryType = taskType;

                                // Try to adapt task type to factory name if
                                // needed
                                if (!factory.isTaskRegistered(factoryType)) {
                                    // Try removing "Task" suffix (e.g.
                                    // TakeExposureTask -> TakeExposure)
                                    if (factoryType.size() > 4 &&
                                        factoryType.rfind("Task") ==
                                            factoryType.size() - 4) {
                                        std::string stripped =
                                            factoryType.substr(
                                                0, factoryType.size() - 4);
                                        if (factory.isTaskRegistered(
                                                stripped)) {
                                ...
                                if (factory.isTaskRegistered(factoryType)) {
                                    auto task = factory.createTask(
                                        factoryType, info->id, info->params);
                                    if (task) {
                                    ...
                                    throw std::invalid_argument(
                                        "Unsupported or unregistered camera "
                                        "task type: " +
                                        taskType);
                                }
                            });
            ...
            .methods("POST"_method)([](const crow::request& req) {
                ...
                return handleTaskAction(
                    ...
                    [&body]() -> nlohmann::json {
                        ...
                        if (!body.has("exposure")) {
                            throw std::invalid_argument(
                                "Missing required parameter: exposure");
                        }
                        ...
                        std::string id = task_manager_->submitTask(
                            "TakeExposure", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("TakeExposure", info->id,
                                                    info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Exposure task submitted for asynchronous "
                            "execution";
        ...
        CROW_ROUTE(app, "/api/tasks/camera/exposures")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createMultipleExposuresTask",
                    [&body]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        if (!body.has("exposure") || !body.has("count")) {
                            throw std::invalid_argument(
                                "Missing required parameters: exposure, count");
                        }
                        ...
                        std::string id = task_manager_->submitTask(
                            "TakeManyExposure", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("TakeManyExposure",
                                                    info->id, info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Multiple exposures task submitted for "
        ...
        CROW_ROUTE(app, "/api/tasks/camera/subframe")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createSubframeExposureTask",
                    [&body]() -> nlohmann::json {
                        nlohmann::json result;

                        // Validate required parameters
                        if (!body.has("exposure") || !body.has("x") ||
                            !body.has("y") || !body.has("width") ||
                            !body.has("height")) {
                            throw std::invalid_argument(
                                "Missing required parameters: exposure, x, y, "
                                "width, height");
                        }
                        ...
                        std::string id = task_manager_->submitTask(
                            "SubframeExposure", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("SubframeExposure",
                                                    info->id, info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Subframe exposure task submitted for asynchronous "
                            "execution";
        ...
        CROW_ROUTE(app, "/api/tasks/camera/settings")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createCameraSettingsTask",
                    [&body]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        std::string id = task_manager_->submitTask(
                            "CameraSettings", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("CameraSettings", info->id,
                                                    info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Camera settings task submitted for asynchronous "
                            "execution";
        ...
        CROW_ROUTE(app, "/api/tasks/camera/preview")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createCameraPreviewTask",
                    [&body]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        std::string id = task_manager_->submitTask(
                            "CameraPreview", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("CameraPreview", info->id,
                                                    info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Camera preview task submitted for asynchronous "
                            "execution";
            ...
            .methods("POST"_method)([](const crow::request& req) {
                ...
                return handleTaskAction(
                    ...
                    [&body]() -> nlohmann::json {
                        ...
                        std::string id = task_manager_->submitTask(
                            "AutoFocus", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("AutoFocus", info->id,
                                                    info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Auto focus task submitted for asynchronous "
                            "execution";
        ...
        CROW_ROUTE(app, "/api/tasks/camera/filter-sequence")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createFilterSequenceTask",
                    [&body]() -> nlohmann::json {
                        nlohmann::json result;

                        if (!body.has("filters") || !body.has("exposure")) {
                            throw std::invalid_argument(
                                "Missing required parameters: filters, "
                                "exposure");
                        }
                        ...
                        std::string id = task_manager_->submitTask(
                            "FilterSequence", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("FilterSequence", info->id,
                                                    info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Filter sequence task submitted for asynchronous "
                            "execution";
        ...
        CROW_ROUTE(app, "/api/tasks/camera/rgb-sequence")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createRGBSequenceTask",
                    [&body]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        std::string id = task_manager_->submitTask(
                            "RGBSequence", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("RGBSequence", info->id,
                                                    info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "RGB sequence task submitted for asynchronous "
                            "execution";
        ...
        CROW_ROUTE(app, "/api/tasks/camera/guided-exposure")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createGuidedExposureTask",
                    [&body]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        std::string id = task_manager_->submitTask(
                            "GuidedExposure", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("GuidedExposure", info->id,
                                                    info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Guided exposure task submitted for asynchronous "
                            "execution";
        ...
        CROW_ROUTE(app, "/api/tasks/camera/auto-calibration")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createAutoCalibrationTask",
                    [&body]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        std::string id = task_manager_->submitTask(
                            "AutoCalibration", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("AutoCalibration", info->id,
                                                    info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Auto calibration task submitted for asynchronous "
                            "execution";
        ...
        CROW_ROUTE(app, "/api/tasks/solver/solve")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createSolverTask", [&body]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        if (body.has("ra"))
                            params["ra"] = body["ra"].d();
                        if (body.has("dec"))
                            params["dec"] = body["dec"].d();
                        if (body.has("scale"))
                            params["scale"] = body["scale"].d();
                        if (body.has("radius"))
                            params["radius"] = body["radius"].d();
                        ...
                        std::string id = task_manager_->submitTask(
                            "SolverTask", params, [](const auto& info) {
                                auto task =
                                    lithium::task::TaskFactory::getInstance()
                                        .createTask("SolverTask", info->id,
                                                    info->params);
                                task->execute(info->params);
                            });

                        result["message"] =
                            "Solver task submitted for asynchronous execution";
        ...
        CROW_ROUTE(app, "/api/tasks/status/<string>")
            .methods("GET"_method)([](const crow::request& req,
                ...
                return handleTaskAction(
                    req, body, "getTaskStatus", [&taskId]() -> nlohmann::json {
                        nlohmann::json result;
                        if (!task_manager_) {
                            throw std::runtime_error(
                                "TaskManager is not initialized");
                        ...
                        result["taskStatus"] = taskStatusToString(info->status);
                        ...
                        if (!info->result.is_null()) {
                            result["result"] =
                                crow::json::load(info->result.dump());
                        }
        ...
        CROW_ROUTE(app, "/api/tasks/active")
            .methods("GET"_method)([](const crow::request& req) {
                auto body = crow::json::load("{}");
                return handleTaskAction(
                    req, body, "getActiveTasks", []() -> nlohmann::json {
                        nlohmann::json result;
                        if (!task_manager_) {
                            throw std::runtime_error(
                                "TaskManager is not initialized");
                        ...
                        std::vector<nlohmann::json> taskList;
                        ...
                        for (const auto& t : active) {
                            ...
                            item["taskStatus"] = taskStatusToString(t->status);
                            item["cancelRequested"] = t->cancelRequested.load();
        ...
        CROW_ROUTE(app, "/api/tasks/cancel/<string>")
            .methods("DELETE"_method)(
                [](const crow::request& req, const std::string& taskId) {
                    auto body = crow::json::load("{}");
                    return handleTaskAction(
                        req, body, "cancelTask", [&taskId]() -> nlohmann::json {
                            nlohmann::json result;
                            if (!task_manager_) {
                                throw std::runtime_error(
                                    "TaskManager is not initialized");
                            ...
                            result["taskId"] = taskId;
                            result["requested"] = ok;
                            result["message"] = ok ? "Cancellation requested"
                                                   : "Task not found";
        ...
        CROW_ROUTE(app, "/api/tasks/device")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createDeviceTask", [&body]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        std::string id = task_manager_->submitTask(
                            "DeviceTask", params, [](const auto& info) {
        ...
        CROW_ROUTE(app, "/api/tasks/script")
            .methods("POST"_method)([](const crow::request& req) {
                auto body = crow::json::load(req.body);
                return handleTaskAction(
                    req, body, "createScriptTask", [&body]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        if (body.has("scriptContent")) {
                            params["scriptContent"] = body["scriptContent"].s();
                        }
                        ...
                        std::string id = task_manager_->submitTask(
                            "ScriptTask", params, [](const auto& info) {
                                using lithium::task::TaskFactory;
                                auto& factory = TaskFactory::getInstance();
                                auto task = factory.createTask(
                                    "script_task", info->id,
        ...
        CROW_ROUTE(app, "/api/tasks/config")
            .methods("POST"_method)([](const crow::request& req) {
                ...
                return handleTaskAction(
                    req, body, "createConfigTask", [&req]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        std::string id = task_manager_->submitTask(
                            "ConfigTask", params, [](const auto& info) {
                                using lithium::task::TaskFactory;
                                auto& factory = TaskFactory::getInstance();
                                auto task = factory.createTask(
                                    "config_task", info->id,
        ...
        CROW_ROUTE(app, "/api/tasks/search")
            .methods("POST"_method)([](const crow::request& req) {
                auto body =
                    crow::json::load(req.body.empty() ? "{}" : req.body);
                return handleTaskAction(
                    req, body, "createSearchTask", [&req]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        std::string id = task_manager_->submitTask(
                            "SearchTask", params, [](const auto& info) {
                                using lithium::task::TaskFactory;
                                auto& factory = TaskFactory::getInstance();
                                ...
                                if (!taskBase) {
                                    throw std::runtime_error(
                                        "Failed to create CelestialSearch "
                                        "task");
                                }
        ...
        CROW_ROUTE(app, "/api/tasks/types")
            .methods("GET"_method)([](const crow::request& req) {
                auto body = crow::json::load("{}");
                return handleTaskAction(
                    req, body, "getTaskTypes", []() -> nlohmann::json {
                        nlohmann::json result;
        ...
        CROW_ROUTE(app, "/api/tasks/schema")
            .methods("GET"_method)([](const crow::request& req) {
                auto body = crow::json::load("{}");
                return handleTaskAction(
                    req, body, "getTaskSchema", [&req]() -> nlohmann::json {
                        nlohmann::json result;
                        ...
                        result["parameterSchema"] =
                            crow::json::load(info.parameterSchema.dump());
