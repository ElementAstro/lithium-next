...
private:
    ...
    crow::response restartService(const crow::request& /*req*/, const std::string& serviceId) {
        using namespace utils;
    ...
    crow::response shutdown(const crow::request& req) {
        ...
        try {
            ...
            std::string reason = body.value("reason", "User requested shutdown");
            bool force = body.value("force", false);

            nlohmann::json data = {
                {"shutdownTime", getCurrentTimestamp(delay)},
                {"activeOperations", 2}
            };

            return ResponseBuilder::accepted("System shutdown scheduled.", data);
        } catch (const nlohmann::json::exception& e) {
            return ResponseBuilder::invalidJson(e.what());
        }
    }
    ...
    crow::response restart(const crow::request& req) {
        ...
        try {
            ...
            nlohmann::json data = {
                {"restartTime", getCurrentTimestamp(delay)},
    ...
    crow::response cancelShutdown(const crow::request& /*req*/) {
        using namespace utils;
        return ResponseBuilder::successWithMessage("Scheduled shutdown/restart cancelled.");
    }
    ...
    crow::response discoverDevices(const crow::request& req) {
        ...
        try {
            ...
            return ResponseBuilder::accepted("Device discovery initiated.", data);
        } catch (const nlohmann::json::exception& e) {
            return ResponseBuilder::invalidJson(e.what());
        }
    }

    crow::response getDiscoveryResults(const crow::request& /*req*/, const std::string& discoveryId) {
        using namespace utils;
        
        nlohmann::json data = {
            {"discoveryId", discoveryId},
            {"status", "completed"},
            {"discoveredDevices", nlohmann::json::array({
                {
                    {"deviceType", "camera"},
                    {"deviceId", "cam-001"},
                    ...
                    {"driver", "ASCOM"},
                    {"interface", "USB"},
                    {"capabilities", nlohmann::json::array({"cooling", "mechanical_shutter"})}
                }
            })},
            {"totalFound", 1},
            ...
            {"drivers", nlohmann::json::array({
                {
                    ...
                    {"version", "6.6.0"},
                    {"type", "platform"},
                    {"supportedDevices", nlohmann::json::array({"camera", "mount", "focuser", "filterwheel", "dome", "rotator"})},
                    {"isInstalled", true}
                },
                {
                    {"name", "INDI"},
                    {"version", "1.9.9"},
                    {"type", "platform"},
                    {"supportedDevices", nlohmann::json::array({"camera", "mount", "focuser", "filterwheel", "dome"})},
                    {"isInstalled", true}
                }
            })}
        };
    ...
    crow::response createBackup(const crow::request& req) {
        ...
        try {
            ...
            return ResponseBuilder::accepted("Backup creation initiated.", data);
        } catch (const nlohmann::json::exception& e) {
            return ResponseBuilder::invalidJson(e.what());
        }
    }
    ...
    crow::response restoreBackup(const crow::request& req) {
        ...
        try {
            ...
            return ResponseBuilder::accepted("Configuration restore initiated.", data);
        } catch (const nlohmann::json::exception& e) {
            return ResponseBuilder::invalidJson(e.what());
        }
    }
