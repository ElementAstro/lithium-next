...
inline auto makeCameraSummary(const std::string &deviceId,
                              const std::string &name, bool isConnected)
    -> json {
    return json{
        {"deviceId", deviceId}, {"name", name}, {"isConnected", isConnected}};
}
...
inline auto makeCameraStatusData(AtomCamera &camera,
    ...
    if (auto binning = camera.getBinning()) {
        data["binning"] =
            json{{"x", binning->horizontal}, {"y", binning->vertical}};
    }
    ...
    json sensor;
    sensor["resolution"] =
        json{{"width", frameInfo.width}, {"height", frameInfo.height}};
...
inline auto makeCameraCapabilitiesData(AtomCamera &camera) -> json {
    ...
    data["binningModes"] =
        json::array({json{{"x", 1}, {"y", 1}}, json{{"x", 2}, {"y", 2}},
                     json{{"x", 3}, {"y", 3}}, json{{"x", 4}, {"y", 4}}});
    ...
    data["readNoise"] = 2.3;
    data["readoutModes"] =
        json::array({json{{"name", "High Quality"}, {"id", 0}},
                     json{{"name", "Fast"}, {"id", 1}}});

    return data;
}

inline auto makeGainsData(AtomCamera &camera, const std::vector<int> &gains)
    -> json {
    json data;
    data["gains"] = gains;
...
inline auto makeOffsetsData(AtomCamera &camera, const std::vector<int> &offsets)
    -> json {
    json data;
