        ...
        try {
            ...
            for (const auto& [type, deviceList] : devices) {
                for (const auto& device : deviceList) {
                    ...
                    deviceInfo["uuid"] = device->getUUID();
                    deviceInfo["connected"] = device->isConnected();

                    // Add metadata if available
                    auto meta = getDeviceManager().getDeviceMetadata(device->getName());
                    if (meta) {
                        deviceInfo["metadata"] = meta->toJson();
                    }

                    // Add state if available
                    auto state = getDeviceManager().getDeviceState(device->getName());
                    if (state) {
                        deviceInfo["state"] = state->toJson();
                    }
    ...
    dispatcher->registerCommand<json>("device.connect", [](json& payload) {
        ...
        try {
            if (payload.value("async", false)) {
                auto future = getDeviceManager().connectDeviceAsync(name, timeout);
                // For async, return immediately with pending status
                json result;
                result["status"] = "pending";
                result["device"] = name;
                result["message"] = "Connection initiated asynchronously";
                payload = CommandResponse::success(result);
            } else {
    ...
    dispatcher->registerCommand<json>("device.disconnect", [](json& payload) {
        ...
        try {
            if (payload.value("async", false)) {
                ...
                result["status"] = "pending";
                result["device"] = name;
                result["message"] = "Disconnection initiated asynchronously";
                payload = CommandResponse::success(result);
            } else {
