...
namespace {
auto& getMountServiceInstance() {
    static lithium::device::MountService instance;
    return instance;
}
}  // namespace

auto listMounts() -> json { return getMountServiceInstance().list(); }

auto getMountStatus(const std::string& deviceId) -> json {
    return getMountServiceInstance().getStatus(deviceId);
}

auto connectMount(const std::string& deviceId, bool connected) -> json {
    return getMountServiceInstance().connect(deviceId, connected);
}

auto slewMount(const std::string& deviceId, const json& body) -> json {
    std::string ra = body.value("ra", "");
    std::string dec = body.value("dec", "");
    return getMountServiceInstance().slew(deviceId, ra, dec);
}

auto setTracking(const std::string& deviceId, const json& body) -> json {
    bool enabled = body.value("enabled", true);
    return getMountServiceInstance().setTracking(deviceId, enabled);
}

auto setMountPosition(const std::string& deviceId, const json& body) -> json {
    return getMountServiceInstance().setPosition(deviceId, body);
}

auto pulseGuide(const std::string& deviceId, const json& body) -> json {
    return getMountServiceInstance().pulseGuide(deviceId, body);
}

auto syncMount(const std::string& deviceId, const json& body) -> json {
    std::string ra = body.value("ra", "");
    std::string dec = body.value("dec", "");
    return getMountServiceInstance().sync(deviceId, ra, dec);
}

auto getMountCapabilities(const std::string& deviceId) -> json {
    return getMountServiceInstance().getCapabilities(deviceId);
}

auto setGuideRates(const std::string& deviceId, const json& body) -> json {
    return getMountServiceInstance().setGuideRates(deviceId, body);
}

auto setTrackingRate(const std::string& deviceId, const json& body) -> json {
    return getMountServiceInstance().setTrackingRate(deviceId, body);
}

auto stopMount(const std::string& deviceId) -> json {
    return getMountServiceInstance().stop(deviceId);
}

auto getPierSide(const std::string& deviceId) -> json {
    return getMountServiceInstance().getPierSide(deviceId);
}

auto meridianFlip(const std::string& deviceId) -> json {
    return getMountServiceInstance().performMeridianFlip(deviceId);
}
