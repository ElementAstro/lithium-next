...
namespace {
auto& getDomeServiceInstance() {
    static lithium::device::DomeService instance;
    return instance;
}
}  // namespace

auto listDomes() -> json { return getDomeServiceInstance().list(); }

auto getDomeStatus(const std::string& deviceId) -> json {
    return getDomeServiceInstance().getStatus(deviceId);
}

auto connectDome(const std::string& deviceId, bool connected) -> json {
    return getDomeServiceInstance().connect(deviceId, connected);
}

auto slewDome(const std::string& deviceId, double azimuth) -> json {
    return getDomeServiceInstance().slew(deviceId, azimuth);
}

auto shutterControl(const std::string& deviceId, bool open) -> json {
    return getDomeServiceInstance().shutter(deviceId, open);
}

auto parkDome(const std::string& deviceId) -> json {
    return getDomeServiceInstance().park(deviceId);
}

auto unparkDome(const std::string& deviceId) -> json {
    return getDomeServiceInstance().unpark(deviceId);
}

auto homeDome(const std::string& deviceId) -> json {
    return getDomeServiceInstance().home(deviceId);
}

auto getDomeCapabilities(const std::string& deviceId) -> json {
    return getDomeServiceInstance().getCapabilities(deviceId);
}
