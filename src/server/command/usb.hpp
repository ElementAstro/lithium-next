...
inline long long getUSBSpace(const std::string& path) {
    try {
        ...
        LOG_F(ERROR, "getUSBSpace: Filesystem error: {}", e.what());
        return -1;
    }
}
...
inline long long getTotalSize(const std::vector<std::string>& paths) {
    ...
    for (const auto& path : paths) {
        try {
            ...
            LOG_F(ERROR, "getTotalSize: Filesystem error: {}", e.what());
        }
    }
    return totalSize;
}
...
inline void usbCheck() {
    ...
    LOG_F(DEBUG, "usbCheck: Checking base path: {}", basePath.string());
    ...
    for (const auto& entry : fs::directory_iterator(basePath)) {
        if (entry.is_directory() && entry.path().filename() != "CDROM") {
            ...
            LOG_F(DEBUG, "usbCheck: Found directory: {}", dirname);
    ...
    if (folderList.size() == 1) {
        ...
        LOG_F(INFO, "usbCheck: Found single USB drive: {}", usbName);
        ...
        if (it == disks.end()) {
            LOG_F(ERROR, "usbCheck: Failed to get space info for USB drive: {}",
        ...
        LOG_F(DEBUG, "usbCheck: Publishing message: {}", message);
        ...
        LOG_F(WARNING, "usbCheck: Multiple USB drives found: count={}",
              folderList.size());
        messageBus->publish("quarcs", "USBCheck:Multiple,Multiple");
    }

    LOG_F(INFO, "usbCheck: Exiting function");
}
}  // namespace lithium::middleware
