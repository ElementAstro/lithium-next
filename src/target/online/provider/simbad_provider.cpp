...
/**
 * @brief Normalize object name for SIMBAD query
 *
...
/**
 * @brief Parse SIMBAD VOTable response
 *
 * Extracts basic SIMBAD fields from VOTable XML response
 *
...
auto parseVotableResponse(const std::string& xml, OnlineQueryResult& result) -> bool {
    try {
        ...
        if (tableStart == std::string::npos || tableEnd == std::string::npos) {
            spdlog::warn("No TABLEDATA found in SIMBAD response");
        ...
        while ((pos = tableContent.find("<TR>", pos)) != std::string::npos) {
            ...
            if (!fields.empty()) {
                ...
                // Map SIMBAD fields to CelestialObjectModel
                ...
                if (fields.size() > 0) {
                    ...
                    obj.mIdentifier = fields[0];  // Use main_id for both
                }
                ...
                if (fields.size() > 5) {
                    // Spectral type - store in aliases for now
                    obj.aliases = fields[5];
                }
        ...
        spdlog::error("Error parsing SIMBAD VOTable response: {}", e.what());
        return false;
    }
}
...
/**
 * @brief Implementation class for SimbadProvider
 */
class SimbadProvider::Impl {
public:
...
// Destructor
SimbadProvider::~SimbadProvider() = default;

// Move constructor
SimbadProvider::SimbadProvider(SimbadProvider&&) noexcept = default;

// Move assignment operator
SimbadProvider& SimbadProvider::operator=(SimbadProvider&&) noexcept = default;

auto SimbadProvider::isAvailable() const -> bool {
    if (!pImpl_ || !pImpl_->httpClient_) {
        spdlog::warn("SIMBAD provider not properly initialized");
        return false;
    }
    ...
    try {
        ...
        if (!response) {
            spdlog::warn("SIMBAD health check failed: {}", response.error());
            return false;
        }

        return response.value().statusCode == 200;
    } catch (const std::exception& e) {
        spdlog::warn("SIMBAD health check error: {}", e.what());
        return false;
    }
}
...
auto SimbadProvider::buildAdqlQuery(const OnlineQueryParams& params) const -> std::string {
    ...
    query << "SELECT main_id, ra, dec, otype, morph_type, sp_type, "
...
auto SimbadProvider::query(const OnlineQueryParams& params)
    ...
    try {
        ...
        // Build ADQL query
        auto adql = buildAdqlQuery(params);
        spdlog::debug("SIMBAD ADQL query: {}", adql);
...
void SimbadProvider::setConfig(const SimbadProviderConfig& config) {
    if (pImpl_) {
        ...
        spdlog::debug("SIMBAD provider configuration updated");
    }
}

auto SimbadProvider::getConfig() const -> const SimbadProviderConfig& {
