...
bool SearchEngine::initializeWithConfig(const EngineConfig& config) {
                spdlog::info(
                    "Initializing SearchEngine with database config: {}",
                    config.databasePath);
                try {
                    ...
                    if (config.useDatabase) {
                        pImpl_->repository_ =
                            std::make_shared<CelestialRepository>(
                                config.databasePath);
                    ...
                    spdlog::error("SearchEngine initialization failed: {}",
                                  e.what());
                    return false;
                }
}
...
int SearchEngine::syncFromJsonFiles() {
                ...
                try {
                    if (!pImpl_->config_.celestialJsonPath.empty()) {
                        ...
                        if (file.is_open()) {
                            auto result = pImpl_->repository_->importFromJson(
                                pImpl_->config_.celestialJsonPath);
                            synced += result.successCount;
                            spdlog::info(
                                "Synced {} objects from celestial JSON",
                                result.successCount);
                        }
                    }
                    ...
                    if (!pImpl_->config_.nameJsonPath.empty()) {
                        ...
                        if (file.is_open()) {
                            ...
                            for (const auto& item : data) {
                                ...
                                std::string name = item[0].get<std::string>();
                                auto existing =
                                    pImpl_->repository_->findByIdentifier(name);

                                if (existing && item.size() >= 2 &&
                                    !item[1].is_null()) {
                                    existing->aliases =
...
std::vector<ScoredSearchResult> SearchEngine::scoredSearch(const std::string& query, int limit) {
                ...
                try {
                    auto exactMatches =
                        pImpl_->repository_->searchByName(query, limit);
                    for (const auto& obj : exactMatches) {
                        ...
                        result.relevanceScore +=
                            std::min(0.2, obj.clickCount * 0.001);
                        results.push_back(result);
                    }

                    if (static_cast<int>(results.size()) < limit) {
                        auto fuzzyMatches = pImpl_->repository_->fuzzySearch(
                            query, pImpl_->config_.fuzzyTolerance,
                            limit - results.size());
                        ...
                        for (const auto& [obj, dist] : fuzzyMatches) {
                            ...
                            result.relevanceScore +=
                                std::min(0.1, obj.clickCount * 0.0005);
                            results.push_back(result);
                        }
                    }
...
std::vector<ScoredSearchResult> SearchEngine::scoredFuzzySearch(
    const std::string& query, int tolerance, int limit) {
                ...
                std::vector<ScoredSearchResult> results;
                try {
                    auto matches = pImpl_->repository_->fuzzySearch(
                        query, tolerance, limit);
                    for (const auto& [obj, dist] : matches) {
                        ...
                        result.editDistance = dist;
                        result.relevanceScore =
                            1.0 - (dist / static_cast<double>(tolerance + 1));
                        results.push_back(result);
                    }
                } catch (const std::exception& e) {
...
std::vector<CelestialObjectModel> SearchEngine::searchByCoordinates(
    double ra, double dec, double radius, int limit) {
                if (!pImpl_->repository_)
                    return {};
                return pImpl_->repository_->searchByCoordinates(ra, dec, radius,
                                                                limit);
}
...
std::vector<CelestialObjectModel> SearchEngine::getByMagnitude(
    double minMag, double maxMag, int limit) {
                if (!pImpl_->repository_)
                    return {};
                return pImpl_->repository_->getByMagnitudeRange(minMag, maxMag,
                                                                limit);
}
                ...
                try {
                    ...
                    if (pImpl_->repository_) {
                        for (const auto& [itemId, score] : recs) {
                            auto obj =
                                pImpl_->repository_->findByIdentifier(itemId);
                            if (obj) {
                    ...
                    spdlog::error("GetModelRecommendations failed: {}",
                                  e.what());
                }
                return results;
}
...
ImportResult SearchEngine::importFromJsonToDb(const std::string& filename) {
                if (!pImpl_->repository_) {
                    return ImportResult{
                        0, 0, 1, 0, {"No repository configured"}};
                }
                pImpl_->clearCache();
...
int64_t SearchEngine::upsertObject(const CelestialObjectModel& obj) {
                ...
                pImpl_->clearCache();
                auto existing =
                    pImpl_->repository_->findByIdentifier(obj.identifier);
                if (existing) {
                    CelestialObjectModel updated = obj;
                    updated.id = existing->id;
                    return pImpl_->repository_->update(updated) ? updated.id
                                                                : -1;
                }
...
void SearchEngine::recordSearch(const std::string& userId, const std::string& query,
                ...
                if (pImpl_->repository_) {
                    pImpl_->repository_->recordSearch(userId, query, searchType,
                                                      resultCount);
                }
}
...
std::string SearchEngine::getStatistics() {
                ...
                if (pImpl_->repository_) {
                    stats["database"] =
                        json::parse(pImpl_->repository_->getStatistics());
                }
