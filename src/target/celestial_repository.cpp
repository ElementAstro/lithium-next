...
public:
    ...
    bool initializeSchema() {
        try {
            ...
            spdlog::error("CelestialRepository: Failed to initialize schema: {}", e.what());
            return false;
        }
    }
    ...
    void createIndexes() {
        const std::vector<std::string> indexSqls = {
            ...
            "CREATE INDEX IF NOT EXISTS idx_celestial_type ON celestial_objects(type)",
            "CREATE INDEX IF NOT EXISTS idx_celestial_magnitude ON celestial_objects(visual_magnitude_v)",
            "CREATE INDEX IF NOT EXISTS idx_celestial_constellation ON celestial_objects(constellation_en)",
            "CREATE INDEX IF NOT EXISTS idx_celestial_coords ON celestial_objects(rad_j2000, dec_d_j2000)",
            "CREATE INDEX IF NOT EXISTS idx_celestial_click ON celestial_objects(click_count DESC)",
            "CREATE INDEX IF NOT EXISTS idx_celestial_aliases ON celestial_objects(aliases)",
        ...
        for (const auto& sql : indexSqls) {
            try {
                ...
                spdlog::warn("CelestialRepository: Index creation warning: {}", e.what());
            }
        }
    }
    ...
    std::optional<CelestialObjectModel> findByIdentifier(const std::string& identifier) {
        try {
            auto stmt = db_->prepare(
                "SELECT * FROM celestial_objects WHERE identifier = ? OR aliases LIKE ?");
            ...
            spdlog::error("CelestialRepository: FindByIdentifier failed: {}", e.what());
            return std::nullopt;
        }
    }
    ...
    std::vector<CelestialObjectModel> searchByName(const std::string& pattern, int limit) {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: SearchByName failed: {}", e.what());
        }
        return results;
    }
    ...
    std::vector<std::pair<CelestialObjectModel, int>> fuzzySearch(
        ...
        try {
            ...
            spdlog::error("CelestialRepository: FuzzySearch failed: {}", e.what());
        }
        return results;
    }
    ...
    std::vector<std::string> autocomplete(const std::string& prefix, int limit) {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: Autocomplete failed: {}", e.what());
        }
        return results;
    }
    ...
    std::vector<CelestialObjectModel> searchByCoordinates(
        ...
        try {
            ...
            spdlog::error("CelestialRepository: SearchByCoordinates failed: {}", e.what());
        }
        return results;
    }
    ...
    std::vector<CelestialObjectModel> getByType(const std::string& type, int limit) {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: GetByType failed: {}", e.what());
        }
        return results;
    }
    ...
    std::vector<CelestialObjectModel> getByMagnitudeRange(
        ...
        try {
            ...
            spdlog::error("CelestialRepository: GetByMagnitudeRange failed: {}", e.what());
        }
        return results;
    }
    ...
    int batchInsert(const std::vector<CelestialObjectModel>& objects, size_t chunkSize) {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: BatchInsert failed: {}", e.what());
        }
        return successCount;
    }
    ...
    int batchUpdate(const std::vector<CelestialObjectModel>& objects, size_t chunkSize) {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: BatchUpdate failed: {}", e.what());
        }
        return successCount;
    }
            ...
            try {
                for (const auto& obj : objects) {
                    ...
                    if (existing) {
                        ...
                        if (update(updated)) ++affectedCount;
                    } else {
                        if (insert(obj) >= 0) ++affectedCount;
                    }
                }
                transaction->commit();
            } catch (...) {
        ...
        try {
            ...
            try {
                for (const auto& item : data) {
                    ...
                    try {
                        CelestialObjectModel obj;
                        obj.identifier = item.value("Identifier", item.value("identifier", ""));
                        obj.mIdentifier = item.value("MIdentifier", item.value("m_identifier", ""));
                        obj.extensionName = item.value("ExtensionName", item.value("extension_name", ""));
                        obj.component = item.value("Component", item.value("component", ""));
                        obj.className = item.value("ClassName", item.value("class_name", ""));
                        obj.amateurRank = item.value("AmateurRank", item.value("amateur_rank", 0));
                        obj.chineseName = item.value("ChineseName", item.value("chinese_name", ""));
                        obj.type = item.value("Type", item.value("type", ""));
                        obj.duplicateType = item.value("DuplicateType", item.value("duplicate_type", ""));
                        obj.morphology = item.value("Morphology", item.value("morphology", ""));
                        obj.constellationZh = item.value("ConstellationZh", item.value("constellation_zh", ""));
                        obj.constellationEn = item.value("ConstellationEn", item.value("constellation_en", ""));
                        obj.raJ2000 = item.value("RAJ2000", item.value("ra_j2000", ""));
                        obj.radJ2000 = item.value("RADJ2000", item.value("rad_j2000", 0.0));
                        obj.decJ2000 = item.value("DecJ2000", item.value("dec_j2000", ""));
                        obj.decDJ2000 = item.value("DecDJ2000", item.value("dec_d_j2000", 0.0));
                        obj.visualMagnitudeV = item.value("VisualMagnitudeV", item.value("visual_magnitude_v", 0.0));
                        obj.photographicMagnitudeB = item.value("PhotographicMagnitudeB", item.value("photographic_magnitude_b", 0.0));
                        obj.bMinusV = item.value("BMinusV", item.value("b_minus_v", 0.0));
                        obj.surfaceBrightness = item.value("SurfaceBrightness", item.value("surface_brightness", 0.0));
                        obj.majorAxis = item.value("MajorAxis", item.value("major_axis", 0.0));
                        obj.minorAxis = item.value("MinorAxis", item.value("minor_axis", 0.0));
                        ...
                        if (options.includeAliases && item.contains("aliases")) {
                            if (item["aliases"].is_array()) {
                                ...
                                for (size_t i = 0; i < item["aliases"].size(); ++i) {
                                    if (i > 0) oss << ",";
                                    oss << item["aliases"][i].get<std::string>();
                                }
                                obj.aliases = oss.str();
                            } else {
                        ...
                        result.errors.push_back("Record " + std::to_string(result.totalRecords) + ": " + e.what());
                        ++result.errorCount;
                    }
                }
                transaction->commit();
            ...
            spdlog::error("CelestialRepository: ImportFromJson failed: {}", e.what());
        }
        return result;
    }
    ...
    ImportResult importFromCsv(const std::string& filename, const ImportExportOptions& options) {
        ...
        try {
                ...
                while (std::getline(file, line)) {
                    ...
                    try {
                        ...
                        for (size_t i = 0; i < headers.size() && i < values.size(); ++i) {
                            ...
                            else if (h == "chinese_name" || h == "ChineseName") obj.chineseName = v;
                            else if (h == "constellation_en" || h == "ConstellationEn") obj.constellationEn = v;
                            else if (h == "ra_j2000" || h == "RAJ2000") obj.raJ2000 = v;
                            else if (h == "dec_j2000" || h == "DecJ2000") obj.decJ2000 = v;
                            else if (h == "rad_j2000" || h == "RADJ2000") obj.radJ2000 = std::stod(v);
                            else if (h == "dec_d_j2000" || h == "DecDJ2000") obj.decDJ2000 = std::stod(v);
                        ...
                        result.errors.push_back("Line " + std::to_string(result.totalRecords) + ": " + e.what());
                        ++result.errorCount;
                    }
                }
                transaction->commit();
            ...
            result.errors.push_back("Import failed: " + std::string(e.what()));
            spdlog::error("CelestialRepository: ImportFromCsv failed: {}", e.what());
        }
        return result;
    }
    ...
    int exportToJson(const std::string& filename, const CelestialSearchFilter& filter,
        ...
        try {
            ...
            spdlog::error("CelestialRepository: ExportToJson failed: {}", e.what());
            return 0;
        }
    }
    ...
    int exportToCsv(const std::string& filename, const CelestialSearchFilter& filter,
        ...
        try {
            ...
            file << "identifier" << options.delimiter
                 ...
                 << "morphology" << options.delimiter
                 << "chinese_name" << options.delimiter
                 << "constellation_en" << options.delimiter
                 << "ra_j2000" << options.delimiter
                 << "dec_j2000" << options.delimiter
                 << "rad_j2000" << options.delimiter
                 << "dec_d_j2000" << options.delimiter
                 << "visual_magnitude_v" << options.delimiter
                 << "click_count";
            ...
            for (const auto& obj : objects) {
                file << obj.identifier << options.delimiter
                     ...
                     << obj.morphology << options.delimiter
                     << obj.chineseName << options.delimiter
                     << obj.constellationEn << options.delimiter
                     << obj.raJ2000 << options.delimiter
                     << obj.decJ2000 << options.delimiter
                     << obj.radJ2000 << options.delimiter
                     << obj.decDJ2000 << options.delimiter
                     << obj.visualMagnitudeV << options.delimiter
                     << obj.clickCount;
                if (options.includeAliases) {
            ...
            spdlog::error("CelestialRepository: ExportToCsv failed: {}", e.what());
            return 0;
        }
    }
    ...
    bool addRating(const std::string& userId, const std::string& objectId, double rating) {
        try {
            ...
            spdlog::error("CelestialRepository: AddRating failed: {}", e.what());
            return false;
        }
    }
    ...
    std::vector<UserRatingModel> getUserRatings(const std::string& userId, int limit) {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: GetUserRatings failed: {}", e.what());
        }
        return results;
    }
    ...
    std::optional<double> getAverageRating(const std::string& objectId) {
        try {
            ...
            spdlog::error("CelestialRepository: GetAverageRating failed: {}", e.what());
            return std::nullopt;
        }
    }
    ...
    std::vector<SearchHistoryModel> getSearchHistory(const std::string& userId, int limit) {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: GetSearchHistory failed: {}", e.what());
        }
        return results;
    }
    ...
    std::vector<std::pair<std::string, int>> getPopularSearches(int limit) {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: GetPopularSearches failed: {}", e.what());
        }
        return results;
    }
    ...
    std::unordered_map<std::string, int64_t> countByType() {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: CountByType failed: {}", e.what());
        }
        return results;
    }
    ...
    bool incrementClickCount(const std::string& identifier) {
        try {
            ...
            spdlog::error("CelestialRepository: IncrementClickCount failed: {}", e.what());
            return false;
        }
    }
    ...
    std::vector<CelestialObjectModel> getMostPopular(int limit) {
        ...
        try {
            ...
            spdlog::error("CelestialRepository: GetMostPopular failed: {}", e.what());
        }
        return results;
    }
...
CelestialRepository::CelestialRepository(std::shared_ptr<database::core::Database> db)
    : pImpl_(std::make_unique<Impl>(std::move(db))) {}

CelestialRepository::~CelestialRepository() = default;

CelestialRepository::CelestialRepository(CelestialRepository&&) noexcept = default;
CelestialRepository& CelestialRepository::operator=(CelestialRepository&&) noexcept = default;

bool CelestialRepository::initializeSchema() { return pImpl_->initializeSchema(); }

int64_t CelestialRepository::insert(const CelestialObjectModel& obj) { return pImpl_->insert(obj); }
bool CelestialRepository::update(const CelestialObjectModel& obj) { return pImpl_->update(obj); }
bool CelestialRepository::remove(int64_t id) { return pImpl_->remove(id); }
std::optional<CelestialObjectModel> CelestialRepository::findById(int64_t id) { return pImpl_->findById(id); }
std::optional<CelestialObjectModel> CelestialRepository::findByIdentifier(const std::string& identifier) { return pImpl_->findByIdentifier(identifier); }

std::vector<CelestialObjectModel> CelestialRepository::searchByName(const std::string& pattern, int limit) { return pImpl_->searchByName(pattern, limit); }
std::vector<std::pair<CelestialObjectModel, int>> CelestialRepository::fuzzySearch(const std::string& name, int tolerance, int limit) { return pImpl_->fuzzySearch(name, tolerance, limit); }
std::vector<CelestialObjectModel> CelestialRepository::search(const CelestialSearchFilter& filter) { return pImpl_->search(filter); }
std::vector<std::string> CelestialRepository::autocomplete(const std::string& prefix, int limit) { return pImpl_->autocomplete(prefix, limit); }
std::vector<CelestialObjectModel> CelestialRepository::searchByCoordinates(double ra, double dec, double radius, int limit) { return pImpl_->searchByCoordinates(ra, dec, radius, limit); }
std::vector<CelestialObjectModel> CelestialRepository::getByType(const std::string& type, int limit) { return pImpl_->getByType(type, limit); }
std::vector<CelestialObjectModel> CelestialRepository::getByMagnitudeRange(double minMag, double maxMag, int limit) { return pImpl_->getByMagnitudeRange(minMag, maxMag, limit); }

int CelestialRepository::batchInsert(const std::vector<CelestialObjectModel>& objects, size_t chunkSize) { return pImpl_->batchInsert(objects, chunkSize); }
int CelestialRepository::batchUpdate(const std::vector<CelestialObjectModel>& objects, size_t chunkSize) { return pImpl_->batchUpdate(objects, chunkSize); }
int CelestialRepository::upsert(const std::vector<CelestialObjectModel>& objects) { return pImpl_->upsert(objects); }

ImportResult CelestialRepository::importFromJson(const std::string& filename, const ImportExportOptions& options) { return pImpl_->importFromJson(filename, options); }
ImportResult CelestialRepository::importFromCsv(const std::string& filename, const ImportExportOptions& options) { return pImpl_->importFromCsv(filename, options); }
int CelestialRepository::exportToJson(const std::string& filename, const CelestialSearchFilter& filter, const ImportExportOptions& options) { return pImpl_->exportToJson(filename, filter, options); }
int CelestialRepository::exportToCsv(const std::string& filename, const CelestialSearchFilter& filter, const ImportExportOptions& options) { return pImpl_->exportToCsv(filename, filter, options); }

bool CelestialRepository::addRating(const std::string& userId, const std::string& objectId, double rating) { return pImpl_->addRating(userId, objectId, rating); }
std::vector<UserRatingModel> CelestialRepository::getUserRatings(const std::string& userId, int limit) { return pImpl_->getUserRatings(userId, limit); }
std::optional<double> CelestialRepository::getAverageRating(const std::string& objectId) { return pImpl_->getAverageRating(objectId); }

void CelestialRepository::recordSearch(const std::string& userId, const std::string& query, const std::string& searchType, int resultCount) { pImpl_->recordSearch(userId, query, searchType, resultCount); }
std::vector<SearchHistoryModel> CelestialRepository::getSearchHistory(const std::string& userId, int limit) { return pImpl_->getSearchHistory(userId, limit); }
std::vector<std::pair<std::string, int>> CelestialRepository::getPopularSearches(int limit) { return pImpl_->getPopularSearches(limit); }

int64_t CelestialRepository::count() { return pImpl_->count(); }
std::unordered_map<std::string, int64_t> CelestialRepository::countByType() { return pImpl_->countByType(); }
bool CelestialRepository::incrementClickCount(const std::string& identifier) { return pImpl_->incrementClickCount(identifier); }
std::vector<CelestialObjectModel> CelestialRepository::getMostPopular(int limit) { return pImpl_->getMostPopular(limit); }

void CelestialRepository::optimize() { pImpl_->optimize(); }
void CelestialRepository::createIndexes() { pImpl_->createIndexes(); }
void CelestialRepository::clearAll(bool includeHistory) { pImpl_->clearAll(includeHistory); }
std::string CelestialRepository::getStatistics() { return pImpl_->getStatistics(); }
