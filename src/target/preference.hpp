...
private:
    ...
    // 缓存相关
    struct Cache {
        std::unordered_map<std::string, std::vector<std::pair<std::string, double>>> recommendations;
    ...
    auto getCachedRecommendations(const std::string& user) -> std::optional<std::vector<std::pair<std::string, double>>>;
    void processBatch(const std::vector<std::tuple<std::string, std::string, double>>& batch);
    auto hybridScore(const std::string& user, const std::string& item) -> double;
