 ...
 */
[[nodiscard]] constexpr std::string_view isolationLevelToString(
    IsolationLevel level) noexcept {
    switch (level) {
...
[[nodiscard]] constexpr std::string_view runnerErrorToString(
    ...
    switch (error) {
        case RunnerError::Success:
            return "Success";
        case RunnerError::ProcessSpawnFailed:
            return "Process spawn failed";
        case RunnerError::ProcessCrashed:
            return "Process crashed";
        case RunnerError::ProcessKilled:
            return "Process killed";
        case RunnerError::HandshakeFailed:
            return "Handshake failed";
        case RunnerError::CommunicationError:
            return "Communication error";
        case RunnerError::Timeout:
            return "Timeout";
        case RunnerError::MemoryLimitExceeded:
            return "Memory limit exceeded";
        case RunnerError::CpuLimitExceeded:
            return "CPU limit exceeded";
        case RunnerError::ExecutionFailed:
            return "Execution failed";
        case RunnerError::Cancelled:
            return "Cancelled";
        case RunnerError::InvalidConfiguration:
            return "Invalid configuration";
        case RunnerError::PythonNotFound:
            return "Python not found";
        case RunnerError::ExecutorNotFound:
            return "Executor script not found";
        case RunnerError::UnknownError:
...
struct IsolationConfig {
    ...
    std::vector<std::filesystem::path>
        allowedPaths;                         ///< Allowed filesystem paths
...
using ProgressCallback = std::function<void(
    float percentage, std::string_view message, std::string_view currentStep)>;

/**
 * @brief Log callback type
 */
using LogCallback =
    std::function<void(std::string_view level, std::string_view message)>;
