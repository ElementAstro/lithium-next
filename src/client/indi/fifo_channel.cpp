...
const std::string& FifoChannel::getFifoPath() const {
...
const FifoChannelConfig& FifoChannel::getConfig() const {
    return config_;
}
...
bool FifoChannel::open() {
    ...
    if (config_.queueCommands && !workerRunning_) {
        workerRunning_ = true;
        workerThread_ = std::make_unique<std::thread>(&FifoChannel::workerThread, this);
    }
...
FifoResult FifoChannel::send(const FifoCommand& command) {
    ...
    if (result.success && command.type == FifoCommandType::Restart) {
        ...
        FifoCommand startCmd = FifoCommand::startDriver(
            command.driverBinary, command.skeletonPath);
...
void FifoChannel::sendAsync(const FifoCommand& command, FifoCommandCallback callback) {
...
uint64_t FifoChannel::getTotalErrors() const {
    return totalErrors_.load();
}

const std::string& FifoChannel::getLastError() const {
    return lastError_;
}
...
FifoResult FifoChannel::writeToFifo(const std::string& data) {
    ...
    if (fifoFd_ < 0) {
        ...
        for (int retry = 0; retry < config_.retryCount; retry++) {
            ssize_t written = write(fd, dataWithNewline.c_str(), 
            ...
            LOG_WARN( "Partial write to FIFO: {} of {} bytes", 
                  written, dataWithNewline.size());
        }
    ...
    for (int retry = 0; retry < config_.retryCount; retry++) {
        ssize_t written = write(fifoFd_, dataWithNewline.c_str(), 
                               dataWithNewline.size());
