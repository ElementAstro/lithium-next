    ...
    def overlay_plot(self, service, outfn, wcsfn, wcsext=0):
        try:
            ...
            if result.get("status") == "success":
                ...
                logger.error(f"Overlay plot failed with status: {result.get('status')}")
        except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--server", default=Client.default_url, help="Set the base server URL"
    )
    parser.add_argument(
        "--apikey",
        "-k",
        help="Astrometry.net API key; if not specified, will check environment variable AN_API_KEY",
    )
    parser.add_argument("--upload", "-u", help="Upload file")
    parser.add_argument("--upload-xy", help="Upload FITS x,y table in JSON format")
    parser.add_argument(
        "--wait", "-w", action="store_true", help="Monitor job status after submission"
    )
    parser.add_argument(
        "--wcs",
        help="Download generated wcs.fits file and save to specified filename; defaults to --wait if using --urlupload or --upload",
    )
    parser.add_argument(
        "--newfits",
        help="Download generated new image fits file and save to specified filename; defaults to --wait if using --urlupload or --upload",
    )
    parser.add_argument(
        "--corr",
        help="Download generated corr.fits file and save to specified filename; defaults to --wait if using --urlupload or --upload",
    )
    parser.add_argument(
        "--kmz",
        help="Download generated kmz file and save to specified filename; defaults to --wait if using --urlupload or --upload",
    )
    parser.add_argument(
        "--annotate",
        "-a",
        help="Store annotation information in specified file in JSON format; defaults to --wait if using --urlupload or --upload",
    )
    parser.add_argument("--urlupload", "-U", help="Upload file from specified URL")
    parser.add_argument(
        "--scale-units",
        choices=["arcsecperpix", "arcminwidth", "degwidth", "focalmm"],
        help="Units for scale estimation",
    )
    parser.add_argument("--scale-lower", type=float, help="Lower bound for scale")
    parser.add_argument("--scale-upper", type=float, help="Upper bound for scale")
    parser.add_argument("--scale-est", type=float, help="Scale estimate")
    parser.add_argument(
        "--scale-err", type=float, help="Scale estimate error (percentage)"
    )
    parser.add_argument("--ra", dest="center_ra", type=float, help="RA center")
    parser.add_argument("--dec", dest="center_dec", type=float, help="Dec center")
    parser.add_argument(
        "--radius", type=float, help="Search radius around RA, Dec center"
    )
    parser.add_argument(
        "--downsample", type=int, help="Downsample image by this factor"
    )
    parser.add_argument(
        "--positional_error",
        type=float,
    ...
    parser.add_argument(
        "--tweak-order", type=int, help="SIP distortion order (default: 2)"
    )
    parser.add_argument(
        "--crpix-center",
        action="store_true",
    ...
    parser.add_argument(
        "--invert", action="store_true", help="Invert image before detecting sources"
    )
    parser.add_argument("--image-width", type=int, help="Set image width for x,y list")
    parser.add_argument(
        "--image-height", type=int, help="Set image height for x,y list"
    )
    parser.add_argument(
        "--album", type=str, help="Add image to album with specified title string"
    )
    parser.add_argument(
        "--sdss",
        nargs=2,
        metavar=("WCSFILE", "OUTFILE"),
        help="Plot SDSS image for given WCS file",
    )
    parser.add_argument(
        "--galex",
        nargs=2,
        metavar=("WCSFILE", "OUTFILE"),
        help="Plot GALEX image for given WCS file",
    )
    parser.add_argument(
        "--jobid",
        "-i",
    ...
    parser.add_argument("--jobs", "-J", action="store_true", help="Get all my jobs")
    parser.add_argument(
        "--jobsbyexacttag", "-T", help="Get list of jobs with exact tag match"
    )
    parser.add_argument(
        "--jobsbytag", "-t", help="Get list of jobs associated with given tag"
    )
    parser.add_argument(
        "--private",
        "-p",
    ...
    parser.add_argument(
        ...
        const="sa",
        default="d",
        help="Allow derivative works under the same license",
    )
    parser.add_argument(
        "--no_mod",
        "-M",
        ...
        const="n",
        default="d",
        help="Do not allow derivative works",
    )
    parser.add_argument(
        "--no_commercial",
        "-c",
    ...
    if opt.upload or opt.urlupload or opt.upload_xy:
        ...
        if opt.upload:
            ...
            try:
                ...
                upload_args.update(x=[float(x) for x in T.x], y=[float(y) for y in T.y])
