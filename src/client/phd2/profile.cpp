...
public:
    ...
    static void saveJsonFile(const fs::path& file_path, const json& config) {
        ...
        if (!file.is_open()) {
            LOG_F(ERROR, "Failed to open JSON file for writing: {}", file_path);
...
auto PHD2ProfileSettingHandler::updateProfile(
    ...
    LOG_F(INFO, "Profile {} updated successfully.", phd2ProfileSetting.name);
    return true;
}
...
auto PHD2ProfileSettingHandler::deleteProfile(
    ...
    if (fs::exists(toDeleteProfileFile)) {
        try {
            ...
            LOG_F(INFO, "Profile {} deleted successfully.", toDeleteProfile);
            return true;
        } catch (const fs::filesystem_error& e) {
            LOG_F(ERROR, "Failed to delete profile {}: {}", toDeleteProfile,
                  e.what());
            return false;
        }
    }
    ...
    if (fs::exists(ServerConfigData::PHD2_HIDDEN_CONFIG_FILE)) {
        try {
            if (fs::exists(profileFile)) {
                ...
                LOG_F(INFO, "Existing profile file {} removed.", profileFile);
            }
            fs::copy_file(ServerConfigData::PHD2_HIDDEN_CONFIG_FILE,
                          profileFile, fs::copy_options::overwrite_existing);
            LOG_F(INFO, "Profile saved successfully as {}.", profileName);
        } catch (const fs::filesystem_error& e) {
    ...
    if (fs::exists(toRestoreFile)) {
        try {
            fs::copy_file(toRestoreFile,
                          ...
                          fs::copy_options::overwrite_existing);
            loadProfileFile();
            LOG_F(INFO, "Profile {} restored successfully.", toRestoreProfile);
            return true;
        } catch (const fs::filesystem_error& e) {
            LOG_F(ERROR, "Failed to restore profile {}: {}", toRestoreProfile,
                  e.what());
            return false;
        }
    } else {
...
auto PHD2ProfileSettingHandler::exportProfile(
    ...
    LOG_F(INFO, "Exporting profile {} to {}", profileName, exportPath);
    fs::path sourceFile =
        pImpl->PHD2_PROFILE_SAVE_PATH / (profileName + ".json");
    if (fs::exists(sourceFile)) {
        try {
            fs::copy_file(sourceFile, exportPath,
                          fs::copy_options::overwrite_existing);
            LOG_F(INFO, "Profile {} exported successfully to {}.", profileName,
                  exportPath);
            return true;
        } catch (const fs::filesystem_error& e) {
    ...
    LOG_F(WARNING, "Profile {} does not exist. Cannot export.", profileName);
    return false;
}

auto PHD2ProfileSettingHandler::importProfile(
    const fs::path& importPath, const std::string& newProfileName) -> bool {
    LOG_F(INFO, "Importing profile from {} as {}", importPath, newProfileName);
    if (fs::exists(importPath)) {
        ...
        try {
            fs::copy_file(importPath, destinationFile,
                          fs::copy_options::overwrite_existing);
            LOG_F(INFO, "Profile imported successfully as {}.", newProfileName);
            return true;
        } catch (const fs::filesystem_error& e) {
            LOG_F(ERROR, "Failed to import profile as {}: {}", newProfileName,
                  e.what());
            return false;
        }
    }
...
auto PHD2ProfileSettingHandler::compareProfiles(
    ...
    LOG_F(INFO, "Comparing profiles: {} and {}", profile1, profile2);
    ...
    try {
        ...
        if (areEqual) {
            ...
            LOG_F(INFO, "Profiles {} and {} have differences.", profile1,
                  profile2);
        }
        return areEqual;
    } catch (const std::exception& e) {
...
auto PHD2ProfileSettingHandler::validateProfile(
    ...
    try {
        ...
        if (!fs::exists(profilePath)) {
            LOG_F(ERROR, "Profile {} does not exist", profileName);
            return false;
        }

        json config = pImpl->loadJsonFile(profilePath);
        // 验证必要字段
        ...
        for (const auto& field : requiredFields) {
            if (!config.contains(json::json_pointer(field))) {
                LOG_F(ERROR, "Missing required field: {} in profile {}", field,
                      profileName);
                return false;
            }
        }
        ...
        LOG_F(ERROR, "Error validating profile {}: {}", profileName, e.what());
        return false;
    }
}
...
auto PHD2ProfileSettingHandler::batchExportProfiles(
    ...
    if (!fs::exists(exportDir)) {
        try {
            ...
            LOG_F(ERROR, "Failed to create export directory: {}", e.what());
            return false;
        }
    }
...
void PHD2ProfileSettingHandler::clearCache() {
    ...
    LOG_F(INFO, "Profile cache cleared");
}
...
auto PHD2ProfileSettingHandler::findProfilesByCamera(
    ...
    try {
        for (const auto& entry :
             fs::directory_iterator(pImpl->PHD2_PROFILE_SAVE_PATH)) {
            if (entry.path().extension() == ".json") {
                json config = pImpl->loadJsonFile(entry.path());
                if (config["profile"]["1"]["indi"]["INDIcam"] == cameraModel) {
                    matches.push_back(entry.path().stem().string());
                }
            }
        }
    } catch (const std::exception& e) {
        LOG_F(ERROR, "Error searching profiles by camera: {}", e.what());
    }
    return matches;
}
