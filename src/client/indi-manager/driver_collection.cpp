...
auto DriverCollection::collectXMLFiles(const std::string& path) -> bool {
...
auto DriverCollection::parseDevice(tinyxml2::XMLElement* device,
                                   const char* family)
...
auto DriverCollection::parseDrivers(const std::string& path) -> bool {
...
auto DriverCollection::parseCustomDrivers(const json& drivers) -> bool {
    ...
    for (const auto& custom : drivers) {
        ...
        drivers_.push_back(std::make_shared<DeviceContainer>(
            name, label, version, binary, family, "", true));
    }
    return true;
}

void DriverCollection::clearCustomDrivers() {
...
auto DriverCollection::getByLabel(const std::string& label)
    -> std::shared_ptr<DeviceContainer> {
...
auto DriverCollection::getByName(const std::string& name)
    -> std::shared_ptr<DeviceContainer> {
...
auto DriverCollection::getByBinary(const std::string& binary)
    -> std::shared_ptr<DeviceContainer> {
...
auto DriverCollection::getFamilies()
