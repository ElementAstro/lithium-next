...
Database::Database(const std::string& filename) : filepath_(filename) {
    ...
    if (std::filesystem::exists(filepath_)) {
        LOG_INFO("Loading existing database from file: {}", filepath_.string());
        load();
    } else {
...
void Database::save() const {
    try {
        ...
        file << db_.dump(2);
        LOG_INFO("Database saved successfully to file: {}", filepath_.string());
    } catch (const std::exception& e) {
...
int Database::addProfile(const std::string& name) {
    ...
    save();
    return static_cast<int>(db_["profiles"].size()) - 1;
}
