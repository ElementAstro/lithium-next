...
Description: INDIHub Agent implementation
...
IndiHubAgent::IndiHubAgent(const std::string& webAddr,
    ...
    : webAddr_(webAddr),
      ...
      asyncCmd_(nullptr),
      commandThread_(nullptr) {
    LOG_INFO("IndiHubAgent created with webAddr: {}, hostname: {}, port: {}",
             webAddr, hostname, port);
}
...
void IndiHubAgent::run(const std::string& profile, const std::string& mode,
    ...
    cmd << "indihub-agent"
        << " -indi-server-manager=" << webAddr_ << " -indi-profile=" << profile
        << " -mode=" << mode << " -conf=" << conf
    ...
    asyncCmd_ = std::make_unique<AsyncSystemCommand>(cmd.str());
    commandThread_ =
        std::make_unique<std::thread>([this]() { asyncCmd_->run(); });
}
...
void IndiHubAgent::stop() {
    ...
    try {
        asyncCmd_->terminate();
        if (commandThread_ && commandThread_->joinable()) {
            commandThread_->join();
        }
        LOG_INFO("IndiHubAgent terminated successfully");
    } catch (const std::exception& e) {
        LOG_WARN("IndiHubAgent termination failed with error: {}", e.what());
    }
}

bool IndiHubAgent::isRunning() const {
    bool running = asyncCmd_ && asyncCmd_->isRunning();
