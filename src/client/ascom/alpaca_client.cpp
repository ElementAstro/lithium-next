...
bool AlpacaClient::isConnected() const {
...
std::optional<AlpacaServerInfo> AlpacaClient::getServerInfo() {
    ...
    if (response.value.is_object()) {
        ...
        info.manufacturerVersion = response.value.value("ManufacturerVersion", "");
...
bool AlpacaClient::connectDevice(ASCOMDeviceType deviceType, int deviceNumber) {
    auto response = put(deviceType, deviceNumber, "connected", {{"Connected", "true"}});
    return response.isSuccess();
}

bool AlpacaClient::disconnectDevice(ASCOMDeviceType deviceType, int deviceNumber) {
    auto response = put(deviceType, deviceNumber, "connected", {{"Connected", "false"}});
    return response.isSuccess();
}

bool AlpacaClient::isDeviceConnected(ASCOMDeviceType deviceType, int deviceNumber) {
...
std::string AlpacaClient::getDeviceName(ASCOMDeviceType deviceType, int deviceNumber) {
...
std::string AlpacaClient::getDeviceDescription(ASCOMDeviceType deviceType, int deviceNumber) {
...
std::string AlpacaClient::getDriverInfo(ASCOMDeviceType deviceType, int deviceNumber) {
...
std::string AlpacaClient::getDriverVersion(ASCOMDeviceType deviceType, int deviceNumber) {
...
int AlpacaClient::getInterfaceVersion(ASCOMDeviceType deviceType, int deviceNumber) {
...
std::vector<std::string> AlpacaClient::getSupportedActions(ASCOMDeviceType deviceType, int deviceNumber) {
...
AlpacaResponse AlpacaClient::get(ASCOMDeviceType deviceType, int deviceNumber,
...
AlpacaResponse AlpacaClient::put(ASCOMDeviceType deviceType, int deviceNumber,
                                  const std::string& method,
                                  const std::unordered_map<std::string, std::string>& params) {
    std::string url = buildUrl(deviceType, deviceNumber, method);
...
std::string AlpacaClient::action(ASCOMDeviceType deviceType, int deviceNumber,
                                  const std::string& actionName, const std::string& parameters) {
...
bool AlpacaClient::commandBool(ASCOMDeviceType deviceType, int deviceNumber,
                                const std::string& command, bool raw) {
    auto response = put(deviceType, deviceNumber, "commandbool",
                        {{"Command", command}, {"Raw", raw ? "true" : "false"}});
    if (response.isSuccess() && response.value.is_boolean()) {
        return response.value.get<bool>();
    }
...
std::string AlpacaClient::commandString(ASCOMDeviceType deviceType, int deviceNumber,
                                         const std::string& command, bool raw) {
    auto response = put(deviceType, deviceNumber, "commandstring",
                        {{"Command", command}, {"Raw", raw ? "true" : "false"}});
    if (response.isSuccess() && response.value.is_string()) {
        return response.value.get<std::string>();
    }
...
void AlpacaClient::setTimeout(int timeout) {
    timeout_ = timeout;
}

int AlpacaClient::getNextTransactionId() {
    return transactionId_.fetch_add(1);
}
...
std::string AlpacaClient::buildUrl(ASCOMDeviceType deviceType, int deviceNumber,
    ...
    oss << "http://" << host_ << ":" << port_ << "/api/v1/"
        << deviceTypeToString(deviceType) << "/" << deviceNumber << "/" << method;
    return oss.str();
}

AlpacaResponse AlpacaClient::executeRequest(HttpMethod method, const std::string& url,
                                             const std::unordered_map<std::string, std::string>& params) {
