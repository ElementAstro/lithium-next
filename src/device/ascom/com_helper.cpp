...
ASCOMCOMHelper::ASCOMCOMHelper()
    : initialized_(false),
      last_hresult_(S_OK),
      property_caching_enabled_(true) {}

ASCOMCOMHelper::~ASCOMCOMHelper() { cleanup(); }
...
bool ASCOMCOMHelper::initialize() {
    ...
    if (FAILED(hr) && hr != RPC_E_TOO_LATE) {
        spdlog::warn("COM security initialization failed: {}",
                     formatCOMError(hr));
    }
...
std::optional<COMObjectWrapper> ASCOMCOMHelper::createObject(
...
std::optional<COMObjectWrapper> ASCOMCOMHelper::createObjectFromCLSID(
    const CLSID& clsid) {
    if (!initialized_) {
        setError("COM not initialized");
        return std::nullopt;
...
std::optional<VariantWrapper> ASCOMCOMHelper::getProperty(
    IDispatch* object, const std::string& property) {
    if (!object) {
    ...
    HRESULT hr = object->Invoke(*dispId, IID_NULL, LOCALE_USER_DEFAULT,
...
bool ASCOMCOMHelper::setProperty(IDispatch* object, const std::string& property,
                                 const VariantWrapper& value) {
    if (!object) {
        setError("Invalid object pointer");
        return false;
    }
...
std::optional<VariantWrapper> ASCOMCOMHelper::invokeMethod(
    IDispatch* object, const std::string& method) {
...
std::optional<VariantWrapper> ASCOMCOMHelper::invokeMethodWithNamedParams(
    IDispatch* object, const std::string& method,
    ...
    HRESULT hr = object->GetIDsOfNames(
        IID_NULL, paramNames.data(), static_cast<UINT>(paramNames.size()),
        LOCALE_USER_DEFAULT, paramDispIds.data());
    ...
    DISPPARAMS dispParams = {variants.data(), paramDispIds.data(),
                             static_cast<UINT>(variants.size()),
                             static_cast<UINT>(paramDispIds.size())};

    VariantWrapper result;
    hr = object->Invoke(*methodDispId, IID_NULL, LOCALE_USER_DEFAULT,
                        DISPATCH_METHOD, &dispParams, &result.get(), nullptr,
                        nullptr);

    if (FAILED(hr)) {
        setError("Failed to invoke method with named parameters: " + method,
                 hr);
        return std::nullopt;
    }
...
bool ASCOMCOMHelper::setMultipleProperties(
    IDispatch* object,
    const std::unordered_map<std::string, VariantWrapper>& properties) {
...
std::unordered_map<std::string, VariantWrapper>
ASCOMCOMHelper::getMultipleProperties(
    IDispatch* object, const std::vector<std::string>& properties) {
...
std::optional<std::vector<VariantWrapper>> ASCOMCOMHelper::safeArrayToVector(
    SAFEARRAY* pArray) {
    if (!pArray) {
        return std::nullopt;
    }
    ...
    for (long i = lBound; i <= uBound; ++i) {
        ...
        switch (vt) {
            case VT_BSTR: {
                ...
                wrapper =
                    VariantWrapper::fromString(_bstr_t(bstrArray[i - lBound]));
                break;
            }
            case VT_I4: {
            ...
            case VT_BOOL: {
                ...
                wrapper = VariantWrapper::fromBool(boolArray[i - lBound] ==
                                                   VARIANT_TRUE);
                break;
            }
            default:
...
std::vector<std::string> ASCOMCOMHelper::enumerateASCOMDrivers(
    ...
    HKEY hKey;
    LONG result =
        RegOpenKeyExA(HKEY_LOCAL_MACHINE, keyPath.c_str(), 0, KEY_READ, &hKey);
    ...
    while (RegEnumKeyExA(hKey, index, subKeyName, &subKeyNameSize, nullptr,
...
std::optional<std::string> ASCOMCOMHelper::getDriverInfo(
    const std::string& progId) {
...
std::optional<DISPID> ASCOMCOMHelper::getDispatchId(IDispatch* object,
    ...
    // Check cache first
    std::string cacheKey =
        std::to_string(reinterpret_cast<uintptr_t>(object)) + ":" + name;
    {
    ...
    DISPID dispId;
    CComBSTR bstrName(name.c_str());
    HRESULT hr = object->GetIDsOfNames(IID_NULL, &bstrName, 1,
...
std::string ASCOMCOMHelper::buildCacheKey(IDispatch* object,
                                          const std::string& property) {
    return std::to_string(reinterpret_cast<uintptr_t>(object)) + ":" + property;
}

std::optional<VariantWrapper> ASCOMCOMHelper::invokeMethodInternal(
    IDispatch* object, DISPID dispId, WORD flags,
    const std::vector<VariantWrapper>& params) {
    std::vector<VARIANT> variants;
    ...
    DISPPARAMS dispParams = {variants.empty() ? nullptr : variants.data(),
                             nullptr, static_cast<UINT>(variants.size()), 0};

    VariantWrapper result;
    HRESULT hr = object->Invoke(dispId, IID_NULL, LOCALE_USER_DEFAULT, flags,
                                &dispParams, &result.get(), nullptr, nullptr);
...
ASCOMDeviceHelper::getSupportedActions() {
    if (!device_object_.isValid()) {
        return std::nullopt;
    }

    auto result =
        com_helper_->getProperty(device_object_.get(), "SupportedActions");
    if (!result) {
        return std::nullopt;
    }
    ...
    if (result->get().vt == (VT_ARRAY | VT_BSTR)) {
        auto vectorResult =
            com_helper_->safeArrayToVector(result->get().parray);
        if (vectorResult) {
...
std::unordered_map<std::string, VariantWrapper>
ASCOMDeviceHelper::discoverCapabilities() {
    ...
    std::vector<std::string> commonProperties = {
        "Name",          "Description",      "DriverInfo",
        "DriverVersion", "InterfaceVersion", "SupportedActions",
        "Connected"};

    return com_helper_->getMultipleProperties(device_object_.get(),
                                              commonProperties);
}
...
void ASCOMDeviceHelper::clearDeviceError() { last_device_error_.clear(); }
