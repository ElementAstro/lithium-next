...
ASCOMTelescope::ASCOMTelescope(std::string name)
    ...
    spdlog::info("ASCOMTelescope constructor called with name: {}", getName());
}

ASCOMTelescope::~ASCOMTelescope() {
    spdlog::info("ASCOMTelescope destructor called");
    disconnect();
...
auto ASCOMTelescope::initialize() -> bool {
    spdlog::info("Initializing ASCOM Telescope");
...
auto ASCOMTelescope::destroy() -> bool {
    spdlog::info("Destroying ASCOM Telescope");
...
auto ASCOMTelescope::connect(const std::string &deviceName, int timeout,
                             int maxRetry) -> bool {
    spdlog::info("Connecting to ASCOM device: {}", deviceName);
...
auto ASCOMTelescope::disconnect() -> bool {
    spdlog::info("Disconnecting ASCOM Telescope");
...
auto ASCOMTelescope::scan() -> std::vector<std::string> {
    spdlog::info("Scanning for ASCOM devices");
...
auto ASCOMTelescope::setPierSide(PierSide side) -> bool {
    ...
    if (connection_type_ == ConnectionType::ALPACA_REST) {
        std::string params =
            "SideOfPier=" + std::to_string(static_cast<int>(side));
...
auto ASCOMTelescope::setTrackRate(TrackMode rate) -> bool {
    ...
    if (connection_type_ == ConnectionType::ALPACA_REST) {
        std::string params =
            "TrackingRate=" + std::to_string(static_cast<int>(rate));
...
auto ASCOMTelescope::enableTracking(bool enable) -> bool {
    ...
    if (connection_type_ == ConnectionType::ALPACA_REST) {
        std::string params =
            "Tracking=" + std::string(enable ? "true" : "false");
...
auto ASCOMTelescope::getTrackRates() -> MotionRates { return motion_rates_; }
...
auto ASCOMTelescope::emergencyStop() -> bool { return abortMotion(); }
...
auto ASCOMTelescope::getTargetRADECJNow()
    -> std::optional<EquatorialCoordinates> {
    return target_radec_;
}

auto ASCOMTelescope::setTargetRADECJNow(double raHours, double decDegrees)
    -> bool {
    return setRADECJNow(raHours, decDegrees);
}

auto ASCOMTelescope::slewToRADECJNow(double raHours, double decDegrees,
    ...
    if (connection_type_ == ConnectionType::ALPACA_REST) {
        ...
        params << "RightAscension=" << std::fixed << std::setprecision(8)
               << raHours << "&Declination=" << std::fixed
               << std::setprecision(8) << decDegrees;

        auto response =
            sendAlpacaRequest("PUT", "slewtocoordinatesasync", params.str());
        if (response) {
...
auto ASCOMTelescope::syncToRADECJNow(double raHours, double decDegrees)
    -> bool {
    if (!isConnected()) {
        return false;
    }
    ...
    if (connection_type_ == ConnectionType::ALPACA_REST) {
        ...
        auto response =
            sendAlpacaRequest("PUT", "synctocoordinates", params.str());
        return response.has_value();
    }
...
// Utility methods
auto ASCOMTelescope::degreesToDMS(double degrees)
...
auto ASCOMTelescope::degreesToHMS(double degrees)
    -> std::tuple<int, int, double> {
...
auto ASCOMTelescope::discoverAlpacaDevices() -> std::vector<std::string> {
    spdlog::info("Discovering Alpaca devices");
    std::vector<std::string> devices;
...
auto ASCOMTelescope::connectToAlpacaDevice(const std::string &host, int port,
                                           int deviceNumber) -> bool {
    spdlog::info("Connecting to Alpaca device at {}:{} device {}", host, port,
                 deviceNumber);
...
auto ASCOMTelescope::disconnectFromAlpacaDevice() -> bool {
    spdlog::info("Disconnecting from Alpaca device");
...
// Helper methods
auto ASCOMTelescope::sendAlpacaRequest(const std::string &method,
...
auto ASCOMTelescope::parseAlpacaResponse(const std::string &response)
    -> std::optional<std::string> {
...
auto ASCOMTelescope::startMonitoring() -> void {
    if (!monitor_thread_) {
        ...
        monitor_thread_ = std::make_unique<std::thread>(
            &ASCOMTelescope::monitoringLoop, this);
    }
}
...
auto ASCOMTelescope::getParkPosition() -> std::optional<EquatorialCoordinates> {
    return std::nullopt;
}
auto ASCOMTelescope::setParkPosition(double ra, double dec) -> bool {
    return false;
}
...
auto ASCOMTelescope::initializeHome(std::string_view command) -> bool {
    return false;
}
...
auto ASCOMTelescope::getSlewRate() -> std::optional<double> {
    return std::nullopt;
}
...
auto ASCOMTelescope::getMoveDirectionEW() -> std::optional<MotionEW> {
    return std::nullopt;
}
auto ASCOMTelescope::setMoveDirectionEW(MotionEW direction) -> bool {
    return false;
}
auto ASCOMTelescope::getMoveDirectionNS() -> std::optional<MotionNS> {
    return std::nullopt;
}
auto ASCOMTelescope::setMoveDirectionNS(MotionNS direction) -> bool {
    return false;
}
auto ASCOMTelescope::startMotion(MotionNS ns_direction, MotionEW ew_direction)
    -> bool {
    return false;
}
auto ASCOMTelescope::stopMotion(MotionNS ns_direction, MotionEW ew_direction)
    -> bool {
    return false;
}

auto ASCOMTelescope::guideNS(int direction, int duration) -> bool {
    return false;
}
auto ASCOMTelescope::guideEW(int direction, int duration) -> bool {
    return false;
}
auto ASCOMTelescope::guidePulse(double ra_ms, double dec_ms) -> bool {
    return false;
}

auto ASCOMTelescope::getAZALT() -> std::optional<HorizontalCoordinates> {
    return std::nullopt;
}
auto ASCOMTelescope::setAZALT(double azDegrees, double altDegrees) -> bool {
    return false;
}
auto ASCOMTelescope::slewToAZALT(double azDegrees, double altDegrees) -> bool {
    return false;
}

auto ASCOMTelescope::getLocation() -> std::optional<GeographicLocation> {
    return std::nullopt;
}
auto ASCOMTelescope::setLocation(const GeographicLocation &location) -> bool {
    return false;
}
auto ASCOMTelescope::getUTCTime()
    -> std::optional<std::chrono::system_clock::time_point> {
    return std::nullopt;
}
auto ASCOMTelescope::setUTCTime(
    const std::chrono::system_clock::time_point &time) -> bool {
    return false;
}
auto ASCOMTelescope::getLocalTime()
    -> std::optional<std::chrono::system_clock::time_point> {
    return std::nullopt;
}
...
auto ASCOMTelescope::setAlignmentMode(AlignmentMode mode) -> bool {
...
auto ASCOMTelescope::getASCOMDriverInfo() -> std::optional<std::string> {
    return driver_info_;
}
auto ASCOMTelescope::getASCOMVersion() -> std::optional<std::string> {
    return driver_version_;
}
auto ASCOMTelescope::getASCOMInterfaceVersion() -> std::optional<int> {
    return interface_version_;
}
auto ASCOMTelescope::setASCOMClientID(const std::string &clientId) -> bool {
    client_id_ = clientId;
    return true;
}
auto ASCOMTelescope::getASCOMClientID() -> std::optional<std::string> {
    return client_id_;
}

// ASCOM capability methods
auto ASCOMTelescope::canPulseGuide() -> bool {
    return ascom_capabilities_.can_pulse_guide;
}
auto ASCOMTelescope::canSetDeclinationRate() -> bool {
    return ascom_capabilities_.can_set_declination_rate;
}
auto ASCOMTelescope::canSetGuideRates() -> bool {
    return ascom_capabilities_.can_set_guide_rates;
}
auto ASCOMTelescope::canSetPark() -> bool {
    return ascom_capabilities_.can_set_park;
}
auto ASCOMTelescope::canSetPierSide() -> bool {
    return ascom_capabilities_.can_set_pier_side;
}
auto ASCOMTelescope::canSetRightAscensionRate() -> bool {
    return ascom_capabilities_.can_set_right_ascension_rate;
}
auto ASCOMTelescope::canSetTracking() -> bool {
    return ascom_capabilities_.can_set_tracking;
}
auto ASCOMTelescope::canSlew() -> bool { return ascom_capabilities_.can_slew; }
auto ASCOMTelescope::canSlewAltAz() -> bool {
    return ascom_capabilities_.can_slew_alt_az;
}
auto ASCOMTelescope::canSlewAltAzAsync() -> bool {
    return ascom_capabilities_.can_slew_alt_az_async;
}
auto ASCOMTelescope::canSlewAsync() -> bool {
    return ascom_capabilities_.can_slew_async;
}
auto ASCOMTelescope::canSync() -> bool { return ascom_capabilities_.can_sync; }
auto ASCOMTelescope::canSyncAltAz() -> bool {
    return ascom_capabilities_.can_sync_alt_az;
}
auto ASCOMTelescope::canUnpark() -> bool {
    return ascom_capabilities_.can_unpark;
}

// Rate methods (placeholder implementations)
auto ASCOMTelescope::getDeclinationRate() -> std::optional<double> {
    return std::nullopt;
}
auto ASCOMTelescope::setDeclinationRate(double rate) -> bool { return false; }
auto ASCOMTelescope::getRightAscensionRate() -> std::optional<double> {
    return std::nullopt;
}
auto ASCOMTelescope::setRightAscensionRate(double rate) -> bool {
    return false;
}
auto ASCOMTelescope::getGuideRateDeclinationRate() -> std::optional<double> {
    return std::nullopt;
}
auto ASCOMTelescope::setGuideRateDeclinationRate(double rate) -> bool {
    return false;
}
auto ASCOMTelescope::getGuideRateRightAscensionRate() -> std::optional<double> {
    return std::nullopt;
}
auto ASCOMTelescope::setGuideRateRightAscensionRate(double rate) -> bool {
    return false;
}
...
auto ASCOMTelescope::connectToCOMDriver(const std::string &progId) -> bool {
    spdlog::info("Connecting to COM driver: {}", progId);
...
auto ASCOMTelescope::disconnectFromCOMDriver() -> bool {
    spdlog::info("Disconnecting from COM driver");
...
auto ASCOMTelescope::invokeCOMMethod(const std::string &method, VARIANT *params,
                                     int param_count)
    ...
    HRESULT hr = com_telescope_->GetIDsOfNames(IID_NULL, &method_name, 1,
    ...
    hr = com_telescope_->Invoke(dispid, IID_NULL, LOCALE_USER_DEFAULT,
...
auto ASCOMTelescope::getCOMProperty(const std::string &property)
    -> std::optional<VARIANT> {
    if (!com_telescope_) {
        return std::nullopt;
    }
    ...
    HRESULT hr = com_telescope_->GetIDsOfNames(IID_NULL, &property_name, 1,
                                               LOCALE_USER_DEFAULT, &dispid);
    if (FAILED(hr)) {
...
auto ASCOMTelescope::setCOMProperty(const std::string &property,
