            ...
            [this](const INDI::PropertyNumber &property) {
                {
                    if (property.isValid()) {
                        ...
                        LOG_INFO("Telescope aperture: {}", telescopeAperture_);
        ...
        device_.watchProperty(
            ...
            [this](const INDI::PropertyNumber &property) {
                if (property.isValid()) {
                    ...
                    LOG_INFO("Park position RA: {}", telescopeParkPositionRA_);
            ...
            [this](const INDI::PropertySwitch &property) {
                if (property.isValid()) {
                    ...
                    for (int i = 0; i < static_cast<int>(property.size());
                        ...
                        if (property[i].getState() == ISS_ON) {
                            LOG_INFO("Slew rate is {}", property[i].getLabel());
...
auto INDITelescope::setTargetRADECJNow(double RAHours, double DECDegree)
    -> bool {
    INDI::PropertyNumber property = device_.getProperty("TARGET_EOD_COORD");
