...
ASCOMClientAdapter::ASCOMClientAdapter(std::shared_ptr<lithium::client::ASCOMClient> client)
...
auto ASCOMClientAdapter::connectServer(const std::string& host, int port) -> bool {
    ...
    if (!client_->connect(target)) {
        LOG_ERROR( "ASCOMClientAdapter: Failed to connect to %s", target.c_str());
        return false;
    }
    
    LOG_INFO( "ASCOMClientAdapter: Connected to server %s:%d", host.c_str(), port);
    return true;
}
...
auto ASCOMClientAdapter::disconnectDevice(const std::string& deviceName) -> bool {
    std::lock_guard<std::mutex> lock(mutex_);
...
auto ASCOMAdapterFactory::createAdapter(std::shared_ptr<lithium::client::ASCOMClient> client)
