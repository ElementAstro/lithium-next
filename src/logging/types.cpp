    ...
    if (j.contains("timestamp")) {
        ...
        if (ts_str.length() >= 19) {
            ...
            if (ts_str.length() > 20 && ts_str[19] == '.') {
                ...
                std::string ms_str = ts_str.substr(dot_pos + 1, end_pos - dot_pos - 1);
                // Pad or truncate to 3 digits
                while (ms_str.length() < 3) ms_str += '0';
                if (ms_str.length() > 3) ms_str = ms_str.substr(0, 3);
                ms = std::stoi(ms_str);
            }
...
auto LoggingConfig::createDefault() -> LoggingConfig {
    ...
    if (config.enable_file) {
        ...
        file_sink.file_path = config.log_dir + "/" + config.log_filename + ".log";
