...
void TakeExposureTask::execute(const json& params) {
    ...
    try {
        ...
        if (time < 0 || time > 7200) {
            THROW_INVALID_ARGUMENT("Exposure time must be between 0 and 7200 seconds");
        }

        LOG_F(INFO, "Starting {} exposure for {} seconds", 
        ...
        if (params.contains("binning")) {
            ...
            camera->setBinning(binning["x"].get<int>(), binning["y"].get<int>());
        }

        camera->startExposure(time);
        
        // Simulate exposure duration
        std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<int>(time * 100))); // Scaled down for simulation
        ...
        LOG_F(INFO, "TakeExposure task completed successfully in {} ms", duration.count());

    } catch (const std::exception& e) {
        LOG_F(ERROR, "TakeExposure task failed: {}", e.what());
        throw;
    }
}
...
void TakeExposureTask::defineParameters(Task& task) {
    task.addParamDefinition("exposure", "number", true, nullptr, "Exposure time in seconds (0-7200)");
    task.addParamDefinition("type", "string", false, "light", "Exposure type: light/dark/bias/flat/snapshot");
    task.addParamDefinition("gain", "number", false, nullptr, "Camera gain (0-1000)");
    task.addParamDefinition("offset", "number", false, nullptr, "Camera offset (0-255)");
    task.addParamDefinition("binning", "object", false, nullptr, "Binning settings {x: int, y: int}");
    task.addParamDefinition("fileName", "string", false, nullptr, "Output filename");
    task.addParamDefinition("path", "string", false, nullptr, "Output directory path");
}
...
void TakeExposureTask::handleCameraError(Task& task, const std::exception& e) {
    ...
    LOG_F(ERROR, "Camera error in TakeExposureTask: {}", e.what());
}
...
void TakeManyExposureTask::execute(const json& params) {
    LOG_F(INFO, "Executing TakeManyExposure task with params: {}", params.dump(4));
    ...
    try {
        ...
        LOG_F(INFO, "Starting {} exposures of {} seconds each with binning {} and gain {} and offset {}",
              count, time, binning, gain, offset);

        for (int i = 0; i < count; ++i) {
            LOG_F(INFO, "Taking exposure {} of {}", i + 1, count);
            
            // Add delay between exposures if specified
            if (params.contains("delay") && i > 0) {
                double delay = params["delay"].get<double>();
                if (delay > 0) {
                    LOG_F(INFO, "Waiting {} seconds before next exposure", delay);
                    std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<int>(delay * 1000)));
                }
            }
            
            TakeExposureTask::execute(params);
            LOG_F(INFO, "Exposure {} of {} completed", i + 1, count);
        }
        
        auto endTime = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
        LOG_F(INFO, "TakeManyExposure task completed {} exposures in {} ms", count, duration.count());
        
    } catch (const std::exception& e) {
        auto endTime = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
        LOG_F(ERROR, "TakeManyExposure task failed after {} ms: {}", duration.count(), e.what());
        throw;
    }
}
...
auto TakeManyExposureTask::createEnhancedTask() -> std::unique_ptr<Task> {
    auto task = std::make_unique<Task>(taskName(), [](const json& params) {
        try {
            ...
            LOG_F(ERROR, "Enhanced TakeManyExposure task failed: {}", e.what());
            throw;
        }
    });
    ...
    task->setTimeout(std::chrono::seconds(3600)); // 1 hour timeout for sequences
...
void TakeManyExposureTask::defineParameters(Task& task) {
    task.addParamDefinition("count", "int", true, 1, "Number of exposures to take");
    task.addParamDefinition("exposure", "double", true, 1.0, "Exposure time in seconds");
    task.addParamDefinition("type", "string", true, "light", "Exposure type (light, dark, bias, flat, snapshot)");
    ...
    task.addParamDefinition("delay", "double", false, 0.0, "Delay between exposures in seconds");
}
...
void TakeManyExposureTask::handleSequenceError(Task& task, const std::exception& e) {
    ...
    LOG_F(ERROR, "Camera sequence task error handled: {}", error);
}
...
void SubframeExposureTask::execute(const json& params) {
    LOG_F(INFO, "Executing SubframeExposure task with params: {}", params.dump(4));
    ...
    try {
        ...
        LOG_F(INFO, "Starting {} subframe exposure for {} seconds at ({},{}) size {}x{} with binning {} and gain {} and offset {}",
              type, time, x, y, width, height, binning, gain, offset);

#ifdef MOCK_CAMERA
        std::shared_ptr<MockCamera> camera = std::make_shared<MockCamera>();
#else
        std::shared_ptr<AtomCamera> camera = GetPtr<AtomCamera>(Constants::MAIN_CAMERA).value();
#endif
        ...
        // Set camera frame
        LOG_F(INFO, "Setting camera frame to ({},{}) size {}x{}", x, y, width, height);
...
#else
        if (camera->getBinning().value() != std::tuple{binning, binning, binning, binning}) {
            LOG_F(INFO, "Setting camera binning to {}x{}", binning, binning);
            camera->setBinning(binning, binning);
        }
#endif
        ...
        // Start exposure
        LOG_F(INFO, "Starting subframe camera exposure for {} seconds", time);
        camera->startExposure(time);
        ...
        while (camera->getExposureStatus()) {
            ...
            if (elapsed > timeoutDuration) {
                LOG_F(ERROR, "Subframe exposure timeout");
        ...
        LOG_F(INFO, "Subframe exposure completed");
        
        LOG_F(INFO, "Saving subframe exposure result");
        camera->saveExposureResult();
        
        auto endTime = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
        LOG_F(INFO, "SubframeExposure task completed in {} ms", duration.count());
        
    } catch (const std::exception& e) {
        auto endTime = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
        LOG_F(ERROR, "SubframeExposure task failed after {} ms: {}", duration.count(), e.what());
        throw;
    }
}
...
auto SubframeExposureTask::createEnhancedTask() -> std::unique_ptr<Task> {
    auto task = std::make_unique<Task>(taskName(), [](const json& params) {
        try {
            ...
            LOG_F(ERROR, "Enhanced SubframeExposure task failed: {}", e.what());
            throw;
        }
    });
...
auto CameraSettingsTask::taskName() -> std::string {
    return "CameraSettings";
}

void CameraSettingsTask::execute(const json& params) {
    LOG_F(INFO, "Executing CameraSettings task with params: {}", params.dump(4));
    ...
    try {
        ...
        LOG_F(INFO, "Setting camera: gain={}, offset={}, binning={}x{}, cooling={}", 
        ...
        if (targetTemp > -999.0 && coolingEnabled) {
            LOG_F(INFO, "Setting camera target temperature to {} °C", targetTemp);
            ...
            LOG_F(INFO, "Disabling camera cooling");
#ifndef MOCK_CAMERA
        ...
        auto endTime = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
        LOG_F(INFO, "CameraSettings task completed in {} ms", duration.count());
        
    } catch (const std::exception& e) {
        auto endTime = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
        LOG_F(ERROR, "CameraSettings task failed after {} ms: {}", duration.count(), e.what());
        throw;
    }
}
...
auto CameraSettingsTask::createEnhancedTask() -> std::unique_ptr<Task> {
    auto task = std::make_unique<Task>(taskName(), [](const json& params) {
        try {
            ...
            LOG_F(ERROR, "Enhanced CameraSettings task failed: {}", e.what());
            throw;
        }
    });
...
void CameraSettingsTask::defineParameters(Task& task) {
    task.addParamDefinition("gain", "int", true, 100, "Camera gain value (0-1000)");
    task.addParamDefinition("offset", "int", true, 10, "Camera offset/brightness value (0-255)");
    task.addParamDefinition("binning", "int", true, 1, "Camera binning factor (1-4)");
    task.addParamDefinition("temperature", "double", false, -999.0, "Target temperature in Celsius");
    task.addParamDefinition("cooling", "bool", false, false, "Enable camera cooling");
}
...
auto CameraPreviewTask::taskName() -> std::string {
...
void CameraPreviewTask::execute(const json& params) {
    ...
    try {
        ...
        LOG_F(INFO, "Starting preview exposure for {} seconds with binning {}x{} and gain {}", 
        ...
        if (autoStretch) {
            LOG_F(INFO, "Applying auto-stretch to preview image");
            // Note: Auto-stretch implementation would go here
        }

        auto endTime = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
        LOG_F(INFO, "CameraPreview task completed in {} ms", duration.count());
        
    } catch (const std::exception& e) {
        auto endTime = std::chrono::steady_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(endTime - startTime);
        LOG_F(ERROR, "CameraPreview task failed after {} ms: {}", duration.count(), e.what());
        throw;
    }
}
...
auto CameraPreviewTask::createEnhancedTask() -> std::unique_ptr<Task> {
    auto task = std::make_unique<Task>(taskName(), [](const json& params) {
        try {
            ...
            LOG_F(ERROR, "Enhanced CameraPreview task failed: {}", e.what());
            throw;
        }
    });
...
void CameraPreviewTask::defineParameters(Task& task) {
    task.addParamDefinition("exposure", "double", false, 1.0, "Preview exposure time in seconds");
    task.addParamDefinition("binning", "int", false, 2, "Camera binning factor for preview");
    task.addParamDefinition("gain", "int", false, 200, "Camera gain value for preview");
    task.addParamDefinition("offset", "int", false, 10, "Camera offset/brightness value");
    task.addParamDefinition("auto_stretch", "bool", false, true, "Apply automatic histogram stretch");
}
...
void CameraPreviewTask::validatePreviewParameters(const json& params) {
    if (params.contains("exposure")) {
        ...
        if (exposure <= 0 || exposure > 60) { // Max 1 minute for preview
            THROW_INVALID_ARGUMENT("Preview exposure time must be between 0 and 60 seconds");
        }
    }
