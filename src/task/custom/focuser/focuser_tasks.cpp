...
void MoveFocuserTask::setupParameters() {
    addParamDefinition("position", "integer", true, nullptr, "Target absolute position");
    addParamDefinition("speed", "integer", false, 100, "Movement speed (1-100%)");
}
...
void MoveFocuserRelativeTask::setupParameters() {
    addParamDefinition("steps", "integer", true, nullptr, "Relative steps (positive=out, negative=in)");
    addParamDefinition("speed", "integer", false, 100, "Movement speed (1-100%)");
}
...
void MoveFocuserRelativeTask::executeImpl(const json& params) {
    ...
    logProgress("Moving focuser " + direction + " by " + std::to_string(std::abs(steps)) + " steps");

    // Simulate movement time
    int moveTime = std::abs(steps) / 10 * 100 / speed;
    std::this_thread::sleep_for(std::chrono::milliseconds(std::max(moveTime, 100)));
