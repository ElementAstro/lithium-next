...
ApiResponse ApiResponse::accepted(const json& data, const std::string& message) {
    ApiResponse response;
...
std::string wsEventTypeToString(WsEventType type) {
    switch (type) {
        case WsEventType::SequenceStart: return "sequence.start";
        case WsEventType::SequenceProgress: return "sequence.progress";
        case WsEventType::SequencePaused: return "sequence.paused";
        case WsEventType::SequenceResumed: return "sequence.resumed";
        case WsEventType::SequenceComplete: return "sequence.complete";
        case WsEventType::SequenceAborted: return "sequence.aborted";
        
        case WsEventType::TargetStart: return "target.start";
        case WsEventType::TargetProgress: return "target.progress";
        case WsEventType::TargetComplete: return "target.complete";
        case WsEventType::TargetFailed: return "target.failed";
        
        case WsEventType::TaskStart: return "task.start";
        case WsEventType::TaskProgress: return "task.progress";
        case WsEventType::TaskComplete: return "task.complete";
        case WsEventType::TaskFailed: return "task.failed";
        
        case WsEventType::ExposureStarted: return "exposure.started";
        case WsEventType::ExposureProgress: return "exposure.progress";
        case WsEventType::ExposureFinished: return "exposure.finished";
        case WsEventType::ExposureAborted: return "exposure.aborted";
        
        case WsEventType::DeviceConnected: return "device.connected";
        case WsEventType::DeviceDisconnected: return "device.disconnected";
        case WsEventType::DeviceStatusUpdate: return "device.status";
        
        case WsEventType::Notification: return "notification";
        case WsEventType::Error: return "error";
...
std::shared_ptr<Target> SequenceConverter::fromApiJson(const json& sequenceJson) {
    try {
        ...
        if (sequenceJson.contains("tasks")) {
            ...
            for (const auto& taskDef : tasks) {
                ...
                for (int i = 0; i < count; ++i) {
                    auto task = factory.createTask(internalType, 
                        internalType + "_" + std::to_string(i), params);
                    if (task) {
...
std::vector<std::string> SequenceConverter::validateSequence(const json& sequenceJson) {
    ...
    for (const auto& task : tasks) {
        ...
        if (taskType == "exposure") {
            if (task.contains("parameters")) {
                ...
                if (!params.contains("duration")) {
                    errors.push_back(prefix + "Exposure requires 'duration' parameter");
                }
            } else {
        ...
        if (taskType == "slew") {
            if (task.contains("parameters")) {
                ...
                if (!params.contains("ra") || !params.contains("dec")) {
                    errors.push_back(prefix + "Slew requires 'ra' and 'dec' parameters");
                }
            } else {
        ...
        if (task.contains("count")) {
            if (!task["count"].is_number_integer() || task["count"].get<int>() < 1) {
                errors.push_back(prefix + "Invalid 'count' value (must be positive integer)");
            }
        }
...
ErrorMapper::ErrorInfo ErrorMapper::mapException(const std::exception& exception) {
    ErrorInfo info;
...
ApiResponse ErrorMapper::createErrorResponse(const std::exception& exception) {
    ...
    return ApiResponse::makeError(info.httpStatus, info.errorCode, 
...
ApiResponse ErrorMapper::createValidationError(const std::string& field,
    ...
    return ApiResponse::makeError(400, "missing_required_field", message, details);
}
