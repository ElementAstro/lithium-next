...
void SubframeExposureTask::execute(const json& params) {
    ...
    std::shared_ptr<ConfigManager> configManager =
        GetPtr<ConfigManager>(Constants::CONFIG_MANAGER).value();
    configManager->set("/lithium/device/camera/x", x);
    configManager->set("/lithium/device/camera/y", y);
    configManager->set("/lithium/device/camera/width", width);
    configManager->set("/lithium/device/camera/height", height);
