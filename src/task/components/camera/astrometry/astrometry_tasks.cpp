...
void PlateSolveExposureTask::setupParameters() {
    ...
    addParamDefinition("search_radius", "number", false, 15.0,
...
void PlateSolveExposureTask::executeImpl(const json& params) {
    ...
    logProgress("Solved: RA=" + std::to_string(solvedRA) +
                "h, Dec=" + std::to_string(solvedDec) +
...
void CenteringTask::setupParameters() {
    ...
    addParamDefinition("target_dec", "number", true, nullptr,
                       "Target Dec (degrees)");
    addParamDefinition("tolerance", "number", false, 10.0,
                       "Centering tolerance (arcsec)");
