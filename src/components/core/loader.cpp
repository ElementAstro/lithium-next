...
auto ModuleLoader::loadModule(std::string_view path, std::string_view name)
    ...
    modInfo->currentStatus = ModuleInfo::Status::LOADING;
    spdlog::debug("Module {} status: {}", name,
                  statusToString(modInfo->currentStatus));
    ...
    try {
        ...
        modInfo->name = toStdString(name);
        modInfo->loadTime = std::chrono::system_clock::now();
        ...
        modInfo->currentStatus = ModuleInfo::Status::LOADED;
        modInfo->enabled.store(true);

        // Update statistics
        updateModuleStatistics(name);

        spdlog::info("Module {} loaded successfully (status: {})", name,
                     statusToString(modInfo->currentStatus));
        return true;
