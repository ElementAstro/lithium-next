...
public:
    ...
    struct Options {
        ...
        lithium::config::ConfigWatcher::WatcherOptions
     ...
     */
    ~ConfigManager();  // Delete copy operations to ensure proper resource
                       // management
    ...
    [[nodiscard]] auto get_as(std::string_view key_path) const
        ...
        try {
            ...
            // Log error about type conversion using spdlog
     ...
     */
    auto set(std::string_view key_path, json &&value)
        -> bool; /**
                  * @brief Sets any compatible value for the specified key path
                  * with validation
                  * @tparam T Type of the value to set (must be compatible with
                  * json)
                  * @param key_path The path to set the configuration value
                  * @param value The value to set
                  * @return bool True if the value was successfully set, false
                  * otherwise
                  * @note This method includes automatic validation and type
                  * checking
                  */
    template <ConfigValue T>
     ...
     * @note This method uses caching for improved performance
     */
    [[nodiscard]] auto has(std::string_view key_path) const
        -> bool; /**
                  * @brief Retrieves all keys in the configuration with caching
                  * @return std::vector<std::string> A vector of keys in the
                  * configuration
                  * @note This method uses caching for improved performance
                  */
     ...
     */
    [[nodiscard]] auto saveAll(const fs::path &dir_path) const
        -> bool; /**
                  * @brief Cleans up the configuration by removing unused
                  * entries and optimizing data
                  * @note This method optimizes cache usage and validates
     ...
     */
    [[nodiscard]] const lithium::config::ConfigWatcher &getWatcher() const;
...
private:
     ...
     */
    void onFileChanged(const fs::path &file_path,
