...
ConfigWatcher::ConfigWatcher(const WatcherOptions& options)
    ...
    logger_->info("ConfigWatcher initialized with poll_interval={}ms, debounce_delay={}ms",
    ...
    if (options_.poll_interval < std::chrono::milliseconds(10)) {
        ...
        const_cast<WatcherOptions&>(options_).poll_interval = std::chrono::milliseconds(10);
    }
...
bool ConfigWatcher::watchFile(const std::filesystem::path& file_path,
    ...
    if (!callback) {
        logger_->error("Cannot watch file '{}': callback is null", file_path.string());
        return false;
    }
    
    if (!std::filesystem::exists(file_path)) {
        logger_->error("Cannot watch file '{}': file does not exist", file_path.string());
        return false;
    }
    
    if (std::filesystem::is_directory(file_path)) {
        logger_->error("Cannot watch file '{}': path is a directory", file_path.string());
        return false;
    }
    ...
    if (watched_paths_.find(key) != watched_paths_.end()) {
        logger_->warn("File '{}' is already being watched", canonical_path.string());
        return true;
    }
    
    try {
        watched_paths_.emplace(key, WatchedPath(canonical_path, std::move(callback), false));
        ...
        logger_->error("Failed to watch file '{}': {}", canonical_path.string(), e.what());
        return false;
    }
}
...
bool ConfigWatcher::watchDirectory(const std::filesystem::path& directory_path,
                                  FileChangeCallback callback) {
    if (!callback) {
        logger_->error("Cannot watch directory '{}': callback is null", directory_path.string());
        return false;
    }
    ...
    if (watched_paths_.find(key) != watched_paths_.end()) {
        logger_->warn("Directory '{}' is already being watched", canonical_path.string());
        return true;
    }
    
    try {
        watched_paths_.emplace(key, WatchedPath(canonical_path, std::move(callback), true));
        ...
        logger_->error("Failed to watch directory '{}': {}", canonical_path.string(), e.what());
        return false;
    }
}
...
bool ConfigWatcher::stopWatching(const std::filesystem::path& path) {
    ...
    try {
        ...
        if (it == watched_paths_.end()) {
            logger_->warn("Path '{}' is not being watched", canonical_path.string());
            return false;
        }
        ...
        logger_->error("Failed to stop watching path '{}': {}", path.string(), e.what());
        return false;
    }
}
...
void ConfigWatcher::updateOptions(const WatcherOptions& options) {
    ...
    options_ = options;
    logger_->info("Updated watcher options: poll_interval={}ms, debounce_delay={}ms",
                  options_.poll_interval.count(), options_.debounce_delay.count());
    ...
    while (running_.load()) {
        ...
        try {
            ...
            for (const auto& [key, watched_path] : paths_copy) {
                if (auto it = watched_paths_.find(key); it != watched_paths_.end()) {
                    it->second.last_write_time = watched_path.last_write_time;
                    it->second.last_event_time = watched_path.last_event_time;
                    it->second.event_count_this_second = watched_path.event_count_this_second;
                }
            }
        ...
        const auto loop_duration = std::chrono::duration_cast<std::chrono::milliseconds>(
            loop_end - loop_start);
...
void ConfigWatcher::checkPath(WatchedPath& watched_path) {
    try {
        if (!std::filesystem::exists(watched_path.path)) {
            if (watched_path.last_write_time != std::filesystem::file_time_type::min()) {
                triggerEvent(watched_path.path, FileEvent::DELETED, watched_path.callback);
                watched_path.last_write_time = std::filesystem::file_time_type::min();
            }
            return;
        }
        ...
        if (watched_path.is_directory) {
            ...
            const auto current_time = std::filesystem::last_write_time(watched_path.path);
            
            if (current_time != watched_path.last_write_time) {
                if (!shouldDebounce(watched_path) && !shouldRateLimit(watched_path)) {
                    const auto event = (watched_path.last_write_time == std::filesystem::file_time_type::min()) 
                                      ? FileEvent::CREATED : FileEvent::MODIFIED;
                    
                    triggerEvent(watched_path.path, event, watched_path.callback);
                }
        ...
        logger_->error("Error checking path '{}': {}", watched_path.path.string(), e.what());
    }
}
...
bool ConfigWatcher::shouldRateLimit(WatchedPath& watched_path) {
    const auto now = std::chrono::steady_clock::now();
    const auto second_boundary = std::chrono::duration_cast<std::chrono::seconds>(
        now.time_since_epoch()).count();
    
    const auto last_second_boundary = std::chrono::duration_cast<std::chrono::seconds>(
        watched_path.last_event_time.time_since_epoch()).count();
    
    if (second_boundary != last_second_boundary) {
        watched_path.event_count_this_second = 0;
    }
    
    if (watched_path.event_count_this_second >= options_.max_events_per_second) {
        std::unique_lock lock(stats_mutex_);
...
bool ConfigWatcher::shouldWatchFile(const std::filesystem::path& path) const {
    ...
    return std::find(options_.file_extensions.begin(), options_.file_extensions.end(), 
...
void ConfigWatcher::processDirectory(WatchedPath& watched_path) {
    try {
        auto process_entry = [&](const std::filesystem::directory_entry& entry) {
            ...
            if (entry.is_regular_file() && shouldWatchFile(entry.path())) {
                try {
                    ...
                    if (current_time > watched_path.last_write_time) {
                        triggerEvent(entry.path(), FileEvent::MODIFIED, watched_path.callback);
                        watched_path.last_write_time = current_time;
                        watched_path.last_event_time = std::chrono::steady_clock::now();
                    }
                } catch (const std::exception& e) {
        ...
        if (options_.recursive) {
            for (const auto& entry : std::filesystem::recursive_directory_iterator(watched_path.path)) {
                process_entry(entry);
            }
        } else {
            for (const auto& entry : std::filesystem::directory_iterator(watched_path.path)) {
                process_entry(entry);
            }
        }
...
void ConfigWatcher::triggerEvent(const std::filesystem::path& path, FileEvent event,
    ...
    try {
        ...
        const auto processing_time = std::chrono::duration_cast<std::chrono::microseconds>(
            stats_.last_event_time - start_time).count() / 1000.0;
        
        if (stats_.total_events_processed == 1) {
            stats_.average_processing_time_ms = processing_time;
        } else {
            stats_.average_processing_time_ms = (stats_.average_processing_time_ms * 
                (stats_.total_events_processed - 1) + processing_time) / stats_.total_events_processed;
        }
        
        static constexpr const char* event_names[] = {"CREATED", "MODIFIED", "DELETED", "MOVED"};
        const auto event_idx = static_cast<size_t>(event);
        const char* event_name = (event_idx < 4) ? event_names[event_idx] : "UNKNOWN";
        
        logger_->debug("File event triggered: {} - {} (processing_time={}ms)",
                       event_name, path.string(), processing_time);
        
    } catch (const std::exception& e) {
        logger_->error("Error in callback for path '{}': {}", path.string(), e.what());
    }
}
