...
#ifndef LITHIUM_CONFIG_COMPONENTS_SERIALIZER_HPP
#define LITHIUM_CONFIG_COMPONENTS_SERIALIZER_HPP
...
public:
    ...
    /**
     * @brief Get current serialization metrics
    ...
    /**
     * @brief Serialization event types for hooks
     */
    enum class SerializerEvent {
        BEFORE_SERIALIZE,    ///< Before serialization starts
        AFTER_SERIALIZE,     ///< After serialization completes
        BEFORE_DESERIALIZE,  ///< Before deserialization starts
        AFTER_DESERIALIZE,   ///< After deserialization completes
        SERIALIZE_ERROR,     ///< Serialization error occurred
        DESERIALIZE_ERROR,   ///< Deserialization error occurred
        FORMAT_DETECTED      ///< Format was auto-detected
    };

    /**
     * @brief Serializer hook callback signature
     * @param event Type of serializer event
     * @param format Serialization format involved
     ...
     * @param hook Callback function for serializer events
     * @return Hook ID for later removal
     */
    size_t addHook(SerializerHook hook);
    ...
    void registerCustomFormat(
        ...
        std::function<std::string(const json&)> serializer,
        std::function<json(std::string_view)> deserializer);

    /**
     * @brief Check if a custom format is registered
     * @param format Format identifier to check
     * @return True if format is registered
     */
    [[nodiscard]] bool hasCustomFormat(const std::string& format) const;

    /**
     * @brief Get list of registered custom formats
     * @return Vector of custom format identifiers
     */
    [[nodiscard]] std::vector<std::string> getCustomFormats() const;
...
using SerializationFormat = lithium::config::SerializationFormat;
using SerializationOptions = lithium::config::SerializationOptions;
using SerializationResult = lithium::config::SerializationResult;
using DeserializationResult = lithium::config::DeserializationResult;
}  // namespace lithium

#endif  // LITHIUM_CONFIG_COMPONENTS_SERIALIZER_HPP
